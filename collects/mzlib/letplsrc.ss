  ...
  (local [(define-struct child (destruct values traversal))
	  ...
	  (define traverse-traversal
		...
		[(number? t) `(if (= ,t ,above-name)
				  (values)
				  (error 'matching "didn't find number: ~a, found: ~a" ,t ,above-name))]
		[(symbol? t) `(if (eq? ,above-name ',t)
				  (values)
				  (error 'matching "didn't find symbol: ~a, found: ~a" ',t ,above-name))]
		[(null? t) `(if (null? ,above-name)
				(values)
				(error 'matching "expected null, found: ~a" ,above-name))]
		   ...
		   (let/ec ,outer-k
		     ,@(map (lambda (pattern-body-pair)
			      (let*-values ([(pattern) (car pattern-body-pair)]
					    [(body) (cadr pattern-body-pair)]
					    [(traversal names-count names)
					     (parse-pattern (translate-pattern pattern))]
  ...
  (define (3-at-x pattern value)
    (unless (equal? 3
		    (with-handlers ([(lambda (x) #t)
		      ...
		      (eval `(rmatch ,value
				     ([,pattern x])))))
      (error '3-at-x "failed with pattern: ~s and value ~s" pattern value)))

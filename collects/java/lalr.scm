...
;;; -------------------------------------------------------------------------
;;; Le présent fichier contient un générateur d'analyseurs syntaxiques 
;;; LALR(1) écrits en Scheme, pour Scheme. À la différence d'autres 
;;; générateurs écrits en Scheme, celui-ci utilise une technique très
;;; rapide pour calculer les ensembles de prévision. L'algorithme utilisé
;;; est le même que celui contenu dans Bison (GNU yacc) et est décrit dans
;;; l'article suivant:
;;;
;;; "Efficient Computation of LALR(1) Look-Ahead Set", F. DeRemer et
;;; T. Pennello, TOPLAS, vol. 4, no. 4, october 1982.
;;;
;;; L'implication immédiate est que le programme n'est pas écrit dans 
;;; un style purement fonctionnel. Les effets de bord sont nombreux,
;;; la rapidité d'exécution ayant été le but recherché.
;;;
;;; Le programme a été testé sur différent systèmes Scheme:
;;;  - scm4e1 sur DEC Alpha, Linux, DOS, Sun, etc.
;;;  - Gambit v2.2 (interprété et compilé)
;;;  - MIT-Scheme 7.2.0 (microcode 11.127, runtime 14.160) (interp. et comp.)
;;; -------------------------------------------------------------------------
;;; L'UTILISATION DU PROGRAMME
;;;
;;; Pour générer un analyseur syntaxique pour une grammaire donnée, il
;;; faut d'abord écrire cette dernière en Scheme. La prochaine section
;;; décrit la syntaxe des grammaires. Supposons donc qu'une telle
;;; grammaire ait été définie par l'expression suivante :
;;;
;;;    (define ma-grammaire { grammaire })
;;;
;;; il suffit d'évaluer
;;;
;;;    (gen-lalr1 ma-grammaire "fichier" [préfixe])
;;;
;;; pour générer l'analyseur. "fichier" est le nom du fichier dans lequel
;;; l'analyseur sera écrit. Le dernier argument, qui est optionnel, 
;;; permet d'ajouter un préfixe au nom de l'analyseur afin de faire
;;; coexister plusieurs analyseurs dans la même application. Ce préfixe
;;; doit être un symbole Scheme, sinon il sera ignoré.
;;;
;;; Pour exécuter l'analyseur syntaxique, il faut d'abord charger le
;;; `driver' d'analyse LR:
;;;
;;;      (load "lr-dvr.scm")
;;;
;;; Pour démarrer l'analyse, il suffit d'évaluer:
;;;
;;;    ([préfixe-]parse lexer errorp)
;;;
;;; où `lexer' est le nom de l'analyseur lexical qui fournira les 
;;; symboles terminaux reconnus à l'analyseur syntaxique. Chaque 
;;; terminal doit être une paire (token . lval) où `token' est
;;; le numéro du terminal et `lval' est un attribut pour ce terminal.
;;; `errorp' est une fonction d'erreur définie par l'usager (la 
;;; procédure standard `error' peut être utilisée par défaut).
;;;
...
;;;     il doit retourner '(0) à chaque fois qu'il est appelé.
;;;
;;;   - le numéro d'un terminal est déterminé par sa position dans 
;;;     la grammaire. Le premier terminal vaut 1, le deuxième 2, etc.
;;;     (Voir l'exemple qui suit).
;;;
;;;   - Le fichier généré, contenant les tables d'analyse, contient
;;;     des déclarations pour les terminaux. Si la grammaire contient
;;;     un terminal `foo', alors le fichier contiendra une déclaration
;;;        (define [préfixe-]foo-tok n)
;;;     où $n$ est la position du terminal dans la grammaire. Ceci
;;;     permet de ne pas avoir à se soucier des numéros associés aux
;;;     terminaux lors de l'écriture de l'analyseur lexical. 
;;;     
;;;   - La fonction d'erreur doit accepter au moins deux arguments, 
;;;     un message d'erreur et le terminal qui a provoqué l'erreur.
;;;     (la procédure `error' accepte un nombre variable d'arguments,
;;;     donc elle en accepte au moins deux).
;;;
;;; -------------------------------------------------------------------------
;;; LE FORMAT DE LA GRAMMAIRE
;;;
;;; La grammaire est spécifiée en donnant d'abord la liste des terminaux 
;;; et ensuite la liste des définitions des catégories (non-terminaux).
;;; Chaque définition de catégorie est une liste dont le premier élément
;;; est le nom de la catégorie et les autres éléments sont les parties
;;; droites des règles associées à la catégorie (des listes de symboles). 
;;; De plus, une action sémantique peut être ajoutée à chaque partie
;;; droite d'une règle.
;;;
...
;;;   f --> ID
;;;
;;; où '+', '*' et ID sont des terminaux
;;;
;;; Cette grammaire, écrite en Scheme, ressemblerait à ceci (avec les
;;; actions sémantiques ajoutées):
;;;
;;; (define ma-grammaire
;;;   '( 
...
;;;    ))
;;;
;;; Dans les actions sémantiques, les symbole $<n> réfère à l'attribut
;;; synthétisé du nième symbole de la production. La valeur de l'attribut
;;; associé au symbole à gauche de la production est le résultat de 
;;; l'évaluation de l'action sémantique. L'action par défaut retourne
...
;;; (Donc, par défaut, l'analyseur construira un arbre d'analyse.)
;;; 
;;; Si on évalue:
;;;
;;;    (gen-lalr1 ma-grammaire "foo.scm" 'mon) 
;;;
;;; alors le fichier "foo.scm" contiendra la définition d'un analyseur
;;; dont le nom sera `mon-parser'.
;;; 
;;; NOTE SUR LA RÉSOLUTION DES CONFLITS
;;; 
;;; Le conflits dans la grammaire sont résolus de manière conventionnelle:
;;; un conflit Shift/Reduce est résolu en un 'shift' et un conflit 
;;; Reduce/Reduce est résolu en faisant une réduction par la règle

(module request-parsing mzscheme
  ...
  (provide/contract
   [struct request ([method symbol?] [uri url?] [headers (listof header?)]
                    ...
                    [host-ip string?] [client-ip string?])]
  ...
  ; close-connection? : (listof (cons symbol string)) number number string string -> boolean
          ...
          (cond
            [(match-method line)
             => (lambda (x)
                  (values
                   (list-ref x 1)
                   (string->url (list-ref x 2))
                   (string->number (list-ref x 3))
                   (string->number (list-ref x 4))))]

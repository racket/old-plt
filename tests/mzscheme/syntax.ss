...
(let ([f
  ...
  (let ([h '(case-lambda
	     [(x y) 'two]
	     [(x y z w) 'four])])
    (error-test (datum->syntax (list h) #f #f) arity?)
    (error-test (datum->syntax (list* h '(1)) #f #f) arity?)
    (error-test (datum->syntax (list* h '(1 2 3)) #f #f) arity?)
    (error-test (datum->syntax (list* h '(1 2 3 4 5 6)) #f #f) arity?)))
...
(define (error-test-let/no-* expr)
  (syntax-test (datum->syntax (cons 'let expr) #f #f))
  (syntax-test (datum->syntax (cons 'let (cons 'name expr)) #f #f))
  (syntax-test (datum->syntax (cons 'letrec expr) #f #f)))
...
(define (do-error-test-let-values/no-* expr syntax-test)
  (syntax-test (datum->syntax (cons 'let-values expr) #f #f))
  (syntax-test (datum->syntax (cons 'letrec-values expr) #f #f)))
...
(let ([wrap (lambda (body)
	      (syntax-test (datum->syntax `(let () ,@body) #f #f))
	      (syntax-test (datum->syntax `(let () (begin ,@body)) #f #f)))])
...
(let ([wrap
	 ...
	 (syntax-test (datum->syntax `(when (positive? 1) ,@body) #f #f))
	 (syntax-test (datum->syntax `(unless (positive? -1) ,@body) #f #f))
	 (syntax-test (datum->syntax `(cond [(positive? 1) ,@body][else #f]) #f #f))
	 (syntax-test (datum->syntax `(cond [(positive? -1) 0][else ,@body]) #f #f))
	 (syntax-test (datum->syntax `(case (positive? 1) [(#t) ,@body][else -12]) #f #f))
	 (syntax-test (datum->syntax `(cond [#t ,@body]) #f #f))
	 (syntax-test (datum->syntax `(do ((x 1)) (#t ,@body) ,@body) #f #f))
	 (syntax-test (datum->syntax `(begin0 12 ,@body) #f #f)))])
...
(define (test-begin bg nested-bg)
  ...
  (syntax-test (datum->syntax `(,bg . 1) #f #f))
  (syntax-test (datum->syntax `(,bg 1 . 2) #f #f))))
...
(define (qq-test e)
  ...
  (syntax-test (datum->syntax (list 'quasiquote e) #f #f))
  (syntax-test (datum->syntax (list 'quasiquote e) #f #f))
  (syntax-test (datum->syntax (list 'quasiquote (list 'quasiquote e)) #f #f))
  (syntax-test (datum->syntax (list 'quasiquote (list 'quasiquote (list 'unquote e))) #f #f))
  (syntax-test (datum->syntax (list 'quasiquote (list 'quasiquote (list 'unquote-splicing e))) #f #f)))
(qq-test #'(unquote))

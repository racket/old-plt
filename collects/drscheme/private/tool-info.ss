...
""
 ...
 "  \\rawscm{read}. "
 "\\item"
 "  \\MzLink{mz:p:current-ps-setup}{\\rawscm{current-ps-setup}} is set"
 "  to a newly created"
 "@link ps-setup"
 "object."
 "\\item The \\MzlibLink{mz:mzlibpconvert}{\\rawscm{current-print-convert-hook}}"
 "is set to a procedure that calls \\rawscm{basic-convert} for each argument"
 "except"
 "@link snip"
 "objects, in which case it just returns them, which overrides the default"
 "behavior of producing `(make-object ...)."

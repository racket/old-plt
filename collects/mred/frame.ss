	...
	(public
	 ...
	 [on-menu-command
	  ...
	  (lambda (op)
	    (cond
	     [(< op font-offset)
	      (send (active-edit) do-edit (- op edit-offset))
	      #t]
	     [(< op other-offset)
	      (send (active-edit) do-font (- op font-offset))
	      #t]
	  ...
	  (lambda ()
	    (let ([file-menu (make-menu)]
	      ...
	      (send file-menu append-item "Open..."
			...
			(if file
			    (open-file file)))))
	      (send file-menu append-item "New File..."
		    (lambda ()
		      (let ((file (mred:finder^:put-file)))
			(if file
			    (open-file file)))))
	      ...
	      (send file-menu append-item "Revert"
		    (lambda () (send (active-edit) load-file)))
	      ...
	      (send file-menu append-item "Save"
		    (lambda () (send (active-edit) save-file)))
	      (send file-menu append-item "Save As..."
		    (lambda () (save-as wx:const-media-ff-same)))
	      ...
	      (send edit-menu append-separator)
	      (set! other-offset
		    (send (active-edit) append-font-items 
			  edit-menu font-offset))

...
(define-signature ballS (make-ball ball-posn ball-speed make-speed move-in-box))
(define-signature goS (go))

(compound-unit/sig (import (PLT : plt:userspace^))
  (link
    [PINGP : pingpDrawS ((require-unit/sig "pingp-lib.ss") PLT)]
    [BALL : ballS
      ((unit/sig ballS (import pingpDrawS plt:userspace^) (include "pingp-test-play.ss"))
       PINGP PLT)]
    (GO : goS

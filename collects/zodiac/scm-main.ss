  ...
  (unit/sig zodiac:scheme-main^
      ...
      (let* ((kwd-1 '(cond else =>))
	      ...
	      (out-pattern-2 (if (language<=? 'structured)
			       '(#%raise (#%make-exn:else
					   ...
					   ((debug-info-handler))))
			       '(#%void)))
	      (in-pattern-3 (if (language<=? 'side-effecting)
			      '()	; will never match
			      '(cond (item) rest ...)))
	      (out-pattern-3 '(or item (cond rest ...)))
	      (in-pattern-4 '(cond (test => receiver) rest ...))
	      (out-pattern-4 '(let ((t test))
				(if t (receiver t) (cond rest ...))))
	      (in-pattern-5 (if (language<=? 'structured)
			      '(cond (test b) rest ...)
			      '(cond (test b0 b1 ...) rest ...)))
	      (out-pattern-5 (if (language<=? 'structured)
			       '(if test b (cond rest ...))
	...
	(lambda (expr env)
	  (or (pat:match-and-rewrite expr m&e-1 out-pattern-1 kwd-1 env)
	    (pat:match-and-rewrite expr m&e-2 out-pattern-2 kwd-1 env)

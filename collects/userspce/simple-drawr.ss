;; simple functions to get figures drawn

(unit/sig userspace:simple-draw^
  (import userspace:error^ graphics^)
  ...
  (define (start WIDTH HEIGHT)
    ...
    (let ((current-window (open-viewport "Canvas" WIDTH HEIGHT)))
      ...
      (set! %clear-solid-line (make-line 'clear-solid-line (clear-line current-window)))
      ;; --- 
      (set! %draw-solid-rect (make-rect 'draw-solid-rect (draw-solid-rectangle current-window)))
      (set! %clear-solid-rect (make-rect 'clear-solid-rect (clear-solid-rectangle current-window)))
      ;; --- 
      (set! %draw-solid-disk (make-circle 'draw-solid-disk (draw-solid-ellipse current-window)))
      (set! %clear-solid-disk (make-circle 'clear-solid-disk (clear-solid-ellipse current-window)))
      ;; ---
      (set! %draw-circle (make-circle 'draw-circle (draw-ellipse current-window)))
      (set! %clear-circle (make-circle 'clear-circle (clear-ellipse current-window)))))

           ...
           ((c-paren? tok)
            (let* ((next (getter))
              ...
              (cond
                ...
                ((o-brace? next-tok) 
                 (parse-members next (parse-ctor-body null (getter) getter) 'ctor-end getter #f))
                ((open-separator? next-tok)
        ...
        ((instanceof)
         (cond
           ...
           ((id-token? tok)
            (let ((next (getter)))
              (if (dot? (get-tok next))
                  (parse-expression next (parse-name (getter) getter #f) 'instanceof-array getter statement-ok?)

...
(define (restart init-argv adjust-flag-table argv init-namespace)
  (let ([args #f]
	[mute-banner? #f]
	[no-rep? #f]
	[no-coll-paths? #f]
    ...
    (parse-command-line
	 ...
	 ((in-parameterization p current-namespace) n)
		   ...
		   (if no-coll-paths?
			...
			(list
			 (lambda () (let ((v (getenv "PLTHOME")))
				      (and v (build-path v "mzscheme" "collects"))))
			 (lambda () (find-executable-path program "collects"))
			 (lambda ()
			   (case (system-type)
			     [(unix) "/usr/local/lib/plt/mzscheme/collects"]
			     [(windows) "c:\\plt\\mzscheme\\collects"]
			     [else #f]))))))

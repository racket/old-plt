...
(module profj-testing mzscheme
  ...
  ;java-equal?: 'a 'a -> bool
  (define (java-equal? v1 v2)
    (cond
      ((and (object? v1) (object? v2))
       (or (eq? v1 v2)
           (and (equal? (send v1 my-name) (send v2 my-name))
                (andmap (lambda (x) x) (map java-equal? (send v1 field-values) (send v2 field-values))))))
      ((and (not (object? v1)) (not (object? v2)))
       (equal? v1 v2))
      (else #f)))
                  ...
                  (with-handlers 
                    ...
                    (let ((new-val (eval `(begin
                                            (require (lib "class.ss")
                                                     (lib "runtime.scm" "profj" "libs" "java"))
                                            ,(compile-interactions st st type-recs level)))))
                      (unless (java-equal? (eval val) new-val)
                        (interaction-errors (add1 (interaction-errors)))
            ...
            (lambda (interact val)
              (with-handlers
                ...
                (let* ((get-val (lambda (v-st v-pe)
                  ...
                  (list 'interact (java-equal? given-val exp-val) (format-java given-val #t 'field null #f 0)))))))
      (with-handlers
          ([exn?
            (lambda (exn)

...
(define out-clean-action
  (lambda (s)
    (let* ((lines (out-split-in-lines s))
      ...
      (list-string-append clean-lines))))

(define (list-string-append l)
  (foldr (lambda (x y) (string-append x y)) "" l))
...
(define out-pp
  (lambda (obj col)
    (let* ((list-rec-of-strings-n-chars
      ...
      (list-string-append list-of-strings))))
    ...
    (letrec ((line-pad
      ...
      (p-obj obj start 0 '() (cons #f #f)
	     (lambda (col objw texts hole)
	       (if (> col out-max-col)
		   (step-line hole))
	       (list-string-append (reverse texts)))))))
...
(define output
  (lambda (args-alist
    ...
    (let* ((fileout          (cdr (assq 'fileout args-alist)))
	   (port             (open-output-file fileout 'replace))

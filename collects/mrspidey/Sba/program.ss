...
(unit/sig mrspidey:program^
  ...
  (define (analyze-program file)
    ...
    (let ([file (normalize-path file)])
      ...
      (do-analyze-program-sexps
       (zodiac:read* (open-code-file file) file)
       (path-only file)
       #t)))

  (define (analyze-program-sexps sexps dir)
    (do-analyze-program-sexps sexps dir #f))

  (define (do-analyze-program-sexps sexps dir needs-expand?)
	...
	(parameterize ([current-load-relative-directory dir])
	  (my-scheme-expand-program sexps needs-expand?))]

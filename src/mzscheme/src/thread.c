...
void scheme_security_check_file(const char *who, const char *filename, int guards)
{
  ...
  if (sg->file_proc) {
    ...
    a[1] = (filename ? scheme_make_immutable_sized_byte_string((char *)filename, -1, 1) : scheme_false);
    a[2] = l;
...
void scheme_security_check_network(const char *who, const char *host, int port, int client)
{
  ...
  if (sg->network_proc) {
    ...
    a[0] = scheme_intern_symbol(who);
    a[1] = (host ? scheme_make_immutable_sized_byte_string((char *)host, -1, 1) : scheme_false);

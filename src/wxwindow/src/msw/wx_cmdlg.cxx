...
Bool wxFontDialog::Show(Bool flag)
{
  if (flag)
  {
    ...
    chooseFontStruct.rgbColors = RGB(fontData.fontColour.Red(), fontData.fontColour.Green(), fontData.fontColour.Blue());
...
void wxFillLogFont(LOGFONT *logFont, wxFont *font)
{
    ...
    HDC dc2 = ::GetDC(NULL);
    int nHeight = font->GetPointSize()*GetDeviceCaps(dc2, LOGPIXELSY)/72;
    ::ReleaseDC(NULL, dc2);
...
//  int lfFamily = logFont->lfPitchAndFamily & 0xF0;
  ...
  HDC dc2 = ::GetDC(NULL);
  fontPoints = 72*logFont->lfHeight/GetDeviceCaps(dc2, LOGPIXELSY);
  ::ReleaseDC(NULL, dc2);

  return new wxFont(fontPoints, fontFamily, fontStyle, fontWeight, fontUnderline);
}

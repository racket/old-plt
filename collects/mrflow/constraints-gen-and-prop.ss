...
(module
         ...
         (let ([label-list (hash-table-map (label-set (label-values-label inflowing-label))
           ...
           (if (= (length label-list) 1)
             ...
             (error 'extend-edge-for-values "values didn't contain list: ~a"
                    (map (lambda (label)
                           (pp-type (get-type label) 'extend-edge-for-values))
                         label-list))
             ))
  ...
  ;(define (process-make-struct-type-primitive var-stx) ...)
...
;               (add-edge-and-propagate-set-through-edge
;                (process-make-struct-type-primitive var-stx)
;                (create-simple-edge bound-label))]
...
;               (add-edge-and-propagate-set-through-edge
;                (process-make-struct-field-accessor var-stx)
;                (create-simple-edge bound-label))]
;              [(eq? var-name 'make-struct-field-mutator)
;               (add-edge-and-propagate-set-through-edge
;                (process-make-struct-field-mutator var-stx)
;                (create-simple-edge bound-label))]
              [else
               (set! *errors*
                     (cons (list (list term)
                        ...
                        (set! *errors*
                              (cons (list (list term)
                                          ...
                                          (format "value ~a not a subtype of union ~a inside application of ~a"
                                                  (pp-type (get-type inflowing-label) 'type-union1)
                                                  ;(syntax-object->datum
                                                  ; (label-term inflowing-label))
  ...
  (define (close-subtypes table type-name)
    (let ([new-type-entry (hash-table-get table type-name *fail-false*)])
      (if new-type-entry
        (car new-type-entry)
      ...
      (lambda (t1 t2 t1-env t2-env delta-flow trace)
        (or
         ...
         (or (and (type-cst? t2) (eq? (type-cst-type t2) 'top))
             ...
             (if (member (cons t1 t2) trace) #t #f))
         ; constants
         (and (type-cst? t1) (type-cst? t2)
              (let* ([t1 (type-cst-type t1)]
                     [t2 (type-cst-type t2)]
                     [t2-entry (hash-table-get subtyping-table t2 *fail-false*)])
                (if t2-entry
              ...
              (subt (type-rec-body t1) t2
                    (append (map (lambda (type-var type)
                                   (cons (type-var-name type-var)
                                         type))
                                 (type-rec-vars t1) (type-rec-types t1))
                            t1-env)
                    t2-env
                    delta-flow trace))
         (and (type-rec? t2)
              (subt t1 (type-rec-body t2)
                    ...
                    (append (map (lambda (type-var type)
                                   (cons (type-var-name type-var)
                                         type))
                                 (type-rec-vars t2) (type-rec-types t2))
                            t2-env)
                    delta-flow trace))
         ; multiple values
  ...
  ; type type (hash-table-of type-flow-var (cons label type)) boolean syntax-object -> boolean
  (define (subtype t1 t2 delta-flow error? term)
  ...
  (define (pp-type type delta-flow)
    (cond
      [(type-empty? type) "_"]
      [(type-cst? type)

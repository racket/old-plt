...
(compound-unit/sig
  ...
  (link
   ...
   (MINI : input-base^ ((unit/sig input-base^
                          (import (foo : framework^))
                          (define frame:basic% foo:frame:basic%)
                          (define text:basic% foo:text:basic%))
                        FRAME))
   (IN : () ((load-relative "input-gui.ss") JVM (MRED : mred^) MINI CH))
   (AWT : () ((load-relative "awt.ss") JVM (MRED : mred^))))
  (export (open GUI)))
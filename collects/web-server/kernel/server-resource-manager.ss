    ...
    (let loop ([server-resource-pool '()]
      ...
      (let ([resource-counter (car msg-wrapper)]
        ...
        (with-handlers ([exn? (lambda (the-exn)
                                (channel-put result-channel the-exn)
                                (loop server-resource-pool (channel-get message-channel)))])
          (cond
            [(request-pool? msg)
             (channel-put result-channel server-resource-pool)
             ...
             (let* ([new-id (create-server-resource-new-id msg)]
                    ...
                    [new-r-src
                     (apply make-a-new-one
                            (cons new-id (cons (+ time-to-live (current-seconds)) args)))]
                    [new-pool
                     (insert new-r-src
                             (kill-old-resources! resource-counter kill-it! server-resource-pool))])
               (channel-put result-channel new-r-src)
            ...
            [(reset-timeout? msg)
             (let* ([r-src (reset-timeout-resource msg)]
               ...
               (let ([new-pool
                      (insert r-src (kill-old-resources! resource-counter kill-it! rest-tree))])
            ...
            [(kill-server-resource? msg)
             (let* ([r-src (kill-server-resource-resource msg)]
               ...
               (let ([new-pool (kill-old-resources! resource-counter kill-it! rest-tree)])
                 (channel-put result-channel #f)
                 (loop new-pool
                       (channel-get message-channel))))])))))

...
(define *int-corresp-to-nul* (- (char->integer #\a) 97))
...
(define *return* (integer->char (+ *int-corresp-to-nul* 13)))

(define *tab* (integer->char (+ *int-corresp-to-nul* 9)))
...
(define get-time-zone (lambda () (or (getenv "TZ") "")))
...
(define table-get
  (lambda (tbl k . d)
    (cond
     ((lassoc k (table.alist tbl) (table.equ tbl)) => cdr)
     ((pair? d) (car d))
     (else #f))))
...
(define write-log
  (lambda (x)
    ...
    (set! *write-log-index*
      (cond
       ((char? x) (if (char=? x #\newline) 0 (+ *write-log-index* 1)))
       ((number? x) (+ *write-log-index* (if (< x 10) 1 2)))
       ((string? x) (+ *write-log-index* (string-length x)))
       (else (+ *write-log-index* 1))))))
...
(define do-message
  (lambda () (write-log (tex-string->html-string (get-group)))))
            ...
            (let loop ((s (list c #\\)))
              (let ((c (snoop-char)))
                (cond
                 ...
                 (else
                  (unless (or *math-mode?* *not-processing?*)
...
(define get-dimen
  (lambda ()
    (let* ((n (get-real)) (c1 (get-actual-char)) (c2 (get-actual-char)))
    ...
    (let ((c (snoop-actual-char)))
      (cond
       ((char=? c *esc-char*)
        (let ((x (get-ctl-seq)))
          (cond
           ((string=? x "\\the") (cadr (find-count (get-ctl-seq))))
           ((string=? x "\\active") 13)
       ...
       ((char=? c #\')
        (get-actual-char)
        (+ *int-corresp-to-nul* (get-integer 8)))
       ((char=? c #\")
        (get-actual-char)
        (+ *int-corresp-to-nul* (get-integer 16)))
       ((char=? c #\`)
        (get-actual-char)
        (ignorespaces)
        (char->integer
          (if (char=? (snoop-actual-char) *esc-char*)
            ...
            (get-actual-char))))
       ((char=? c #\-) (get-actual-char) (- (get-integer 10)))
...
(define get-tex-char-spec
  (lambda ()
    ...
    (case (snoop-actual-char)
      ...
      ((#\')
       (get-actual-char)
       (integer->char (+ *int-corresp-to-nul* (get-integer 8))))
      ((#\")
       (get-actual-char)
       (integer->char (+ *int-corresp-to-nul* (get-integer 16))))
      (else (integer->char (+ *int-corresp-to-nul* (get-integer 10)))))))
...
(define do-css-block
  (lambda ()
    ...
    (let loop ()
      (let ((c (snoop-actual-char)))
        (cond
         ((eof-object? c) (terror 'do-css-block))
         ((char=? c *esc-char*)
          (let ((x (get-ctl-seq)))
            (cond
             ((string=? x "\\endcssblock") 'done)
             ((string=? x "\\end")
              (cond
               ((get-grouped-environment-name-if-any)
                ...
                (lambda (e)
                  (unless (string=? e "cssblock")
                    ...
                    (write-char #\} *css-port*)
                    (loop))))
               (else (display x *css-port*) (loop))))
             (else (display x *css-port*) (loop)))))
         (else (get-actual-char) (write-char c *css-port*) (loop)))))))
...
(define increment-section-counter
  (lambda (seclvl unnumbered?)
    ...
    (table-for-each
      *section-counters*
      (lambda (k v)
        (if (and (> k seclvl) (> k 0)) (table-put! *section-counters* k 0))))
    (for-each
      (lambda (counter-name)
  ...
  (lambda (seclvl)
    (let ((unnumbered?
      ...
      (let* ((htmlnum
        ...
        (emit "<h")
        (emit htmlnum)
        (case seclvl ((-1) (emit " class=part")) ((0) (emit " class=chapter")))
        (emit ">")
        (let ((write-to-toc?
                (and *toc-page*
                     (not
          ...
          (let ((tocdepth (get-gcount "\\tocdepth")))
            (when (and write-to-toc? (or (< tocdepth -1) (<= seclvl tocdepth)))
              (write-aux
                `(!toc-entry
                   ,(if (= seclvl -1)
                      -1
...
(define do-appendix
  (lambda ()
    (unless *inside-appendix?*
      ...
      (table-put! *section-counters* (if *using-chapters?* 0 1) 0))))
                ...
                ((counter.within counter)
                 ...
                 (lambda (sec-num)
                   (string-append
                     ...
                     (number->string new-counter-value))))
                (else (number->string new-counter-value))))
             (fig-tag (string-append "%_fig_" fig-num)))
...
(define do-end-figure
  (lambda ()
    (cond
     ((snoop-actual-char)
      =>
      (lambda (c) (if (char=? c #\*) (get-actual-char)))))
    (do-end-table)))
...
(define do-caption
  (lambda ()
    (get-bracketed-text-if-any)
    (do-end-para)
    (emit "</td></tr><caption align=bottom><div align=")
    (emit *display-justification*)
    (emit "><b>")
  ...
  (lambda (sw)
    ...
    (add-postlude-to-top-frame
      (cond
       ...
       ((string=? sw "\\em") (emit "<em>") (lambda () (emit "</em>")))
       ((ormap (lambda (z) (string=? sw z)) '("\\it" "\\itshape" "\\sl"))
        (emit "<i>")
        (lambda () (emit "</i>")))
       ((string=? sw "\\bf") (emit "<strong>") (lambda () (emit "</strong>")))
       ((string=? sw "\\tt")
        (let ((old-ligatures? *ligatures?*))
          (set! *ligatures?* #f)
          (emit "<tt>")
          (lambda () (emit "</tt>") (set! *ligatures?* old-ligatures?))))
       ((ormap (lambda (z) (string=? sw z)) '("\\sc" "\\scshape"))
        (let ((old-in-small-caps? *in-small-caps?*))
          (set! *in-small-caps?* #t)
          (lambda () (set! *in-small-caps?* old-in-small-caps?))))
       ((ormap
         (lambda (z) (string=? sw z))
...
(define do-obeyspaces
  (lambda () (tex-def-char #\space '() "\\TIIPnbsp" (top-texframe))))
...
(define do-block
  (lambda (z)
    ...
    (emit "<div ")
    (cond
     ((string=? z "\\center") (emit "align=center"))
...
(define do-hskip
  (lambda ()
    (let* ((len (get-dimen)) (qty (car len)) (the-unit (cadr len)))
      (case the-unit
        ((pt) (emit-nbsp (/ qty 5)))
        ((em) (emit-nbsp (* 2 qty)))
        ((in) (emit-nbsp (* 72.27 0.5 qty)))
        (else (emit-nbsp 3))))))
        ...
        (if (< i 0)
          ...
          (let ((c (string-ref lbl i)))
            (cond
             ...
             ((char=? c #\?) (loop (- i 1) #f `(#\_ #\Q #\_ ,@r)))
             ((char=? c #\:) (loop (- i 1) #f `(#\_ #\C #\_ ,@r)))
             ((char=? c #\;) (loop (- i 1) #f `(#\_ #\S #\_ ,@r)))
             ((char=? c #\#) (loop (- i 1) #f `(#\_ #\H #\_ ,@r)))
             ((char=? c #\_) (loop (- i 1) #f `(#\_ #\U #\_ ,@r)))
             (else (loop (- i 1) #f (cons c r))))))))))
...
(define do-tag
  (lambda () (let ((tag-name (get-label))) (do-tag-aux tag-name (get-group)))))
  ...
  (lambda (label ext-file link-text)
    (let ((label-table
            (if ext-file
              ...
              *label-table*)))
      (let ((label-vec (label-bound? label label-table)))
        (if label-vec
          (begin
            (emit "<a href=\"")
            (let ((pageno (vector-ref label-vec 0)))
              (cond
               (ext-file
                (emit ext-file)
                (when (> pageno 0) (emit *html-page-suffix*) (emit pageno))
                (emit ".html#"))
               (else (emit (maybe-label-page pageno)))))
            (emit (vector-ref label-vec 1))
            (emit "\">")
            (tex2page-string (or link-text (vector-ref label-vec 2)))
            (emit "</a>"))
          (non-fatal-error (or link-text label)))))))
...
(define do-urlh
  (lambda ()
    ...
    (emit "\">")
    (fluid-let ((*doing-urlh?* #t)) (tex2page-string (get-group)))
    (emit "</a>")))

(define do-urlhd (lambda () (do-urlh) (get-group)))
        ...
        (let loop ()
          (cond
           ((get-csv)
            =>
            (lambda (key)
              (if first-key? (set! first-key? #f) (emit ",&nbsp;"))
              (write-bib-aux "\\citation{")
              (write-bib-aux key)
              (write-bib-aux "}")
              (write-bib-aux #\newline)
              (do-ref-aux
                (string-append "cite{" (htmlize-label key) "}")
                #f
                #f)
              (loop)))
           (extra-text (emit ",&nbsp;") (tex2page-string extra-text))))
        (unless (char=? (get-actual-char) #\}) (terror 'do-cite "Missing }"))
      ...
      (let loop ((i (if item 1 *remember-index-number*)))
        (cond
         ((get-csv)
          =>
          (lambda (s)
            (let* ((n (string->number s)) (pageno (assv n *index-alist*)))
              (cond
               (pageno
...
(define do-index-item
  (lambda (z)
    ...
    (emit-newline)
    (cond
     ((string=? z "\\indexsubitem") (emit-nbsp 4))
...
(define output-colophon
  (lambda ()
    ...
    (emit "<div class=smallprint>")
    (emit-newline)
    (emit "This document was converted to HTML by TeX2page ")
...
(define do-start
  (lambda ()
    ...
    (emit "<html>")
    (emit-newline)
    (emit "<!-- Generated from TeX source by tex2page, v ")
    (emit *tex2page-version*)
    (emit ", ")
    (emit-newline)
    ...
    (emit-newline)
    (emit "<body")
    (if (null? *stylesheets*) (emit " bgcolor=#ffffff text=#000000"))
    (emit ">")
    (emit-newline)
    (unless (= *html-page-count* 0) (output-navigation-bar))))

(define do-end-page
  (lambda ()
    (output-footnotes)
    (output-navigation-bar)
    (when (= *html-page-count* 0) (output-modification-time))
    (do-end-para)
...
(define do-diacritic
  (lambda (diac)
    (let* ((x (ungroup (get-token)))
      ...
      (cond
       ((and (memv c '(#\a #\e #\i #\o #\u #\A #\E #\I #\O #\U))
             (ormap
              (lambda (z) (string=? diac z))
              '("acute" "uml" "circ" "grave")))
        (emit "&")
        (emit c)
        (emit diac)
        (emit ";"))
       ((and (memv c '(#\a #\n #\o #\A #\N #\O)) (string=? diac "tilde"))
        (emit "&")
        (emit c)
        (emit "tilde;"))
    ...
    (source-img-file
      (call-with-image-port
        (lambda (o)
          ...
          (let ((c (get-actual-char)))
            (cond
             ((eof-object? c) (terror 'do-img-display-math))
             ((char=? c #\$) #t)
  ...
  (lambda ()
    (let ((display? #f))
      ...
      ((if *use-img-for-math?*
         ...
         (if display? do-imgless-display-math do-imgless-in-text-math))))))
           ...
           (when (ctl-seq? two)
             (set! two
               (cond
                ...
                (else 'undefined))))
           (if (eqv? one two) do-iftrue do-iffalse)))))))
...
(define do-htmlimg
  (lambda (env)
    (source-img-file
      (call-with-image-port (lambda (o) (dump-till-end-env env o))))))

(define do-html-img-format
  (lambda () (set! *img-format* (string->symbol (ungroup (get-group))))))
             ...
             (or (not (file-exists? logfile))
                 (let ((i (open-input-file logfile)))
                   (let loop ()
                     (let ((x (read-line i)))
                       (cond
                        ((eof-object? x) (close-input-port i) #t)
...
(define ps-to-img/jpeg
  (lambda (ps-file f)
    ...
    (system (string-append "pnmcrop " f ".ppm.1 > " f ".ppm.tmp"))
    (system (string-append "ppmquant 256 < " f ".ppm.tmp > " f ".ppm"))
    (system (string-append "ppmtojpeg --grayscale < " f ".ppm > " f ".jpeg"))
    (for-each
      (lambda (e) (ensure-file-deleted (string-append f e)))
      '(".ppm.1" ".ppm.tmp" ".ppm"))))
...
(define do-opengraphsfile
  (lambda ()
    (set! *current-mfpic-file-stem* (string-trim-blanks (ungroup (get-group))))
      ...
      (call-with-image-port
        (lambda (o)
          (fluid-let
            ...
            (let ((img-file-stem
                    (string-append
                      *current-mfpic-file-stem*
                      *mfpic-file-suffix*
                      (number->string *current-mfpic-file-stem*))))
              (display "\\input mfpic" o)
              (newline o)
              (display "\\usemetapost" o)
              (newline o)
              ...
              '(let loop ()
                 (let ((c (snoop-actual-char)))
                   (cond
                    ((eof-object? c) (terror 'do-mfpic "Eof inside \\mfpic"))
                    ((char=? c *esc-char*)
                     (let ((x (get-ctl-seq)))
                       (unless (string=? x "\\endmfpic")
                         (display x o)
                         (loop))))
                    (else (display (get-actual-char) o) (loop)))))
              (display "\\endmfpic" o)
              (newline o)
              (display "\\closegraphsfile" o)
              (newline o))))))))
...
(define do-comment (lambda () (eat-till-eol)))

(define latex-style-file?
  (lambda (f) (let ((e (file-extension f))) (and e (string=? e ".sty")))))
...
(define actual-tex-filename
  (lambda (f)
    (or (find-some-file *tex2page-inputs* (string-append f ".tex") f) f)))
             ...
             ((string=? x "\\input")
              (let ((f (get-filename)))
                (call-with-input-file/buffered
                  f
                  (lambda () (do-html-head-only)))
                (loop '())))
             (else (write-aux `(!html-head ,x)) (loop '())))))
         (else (get-actual-char) (loop (cons c s))))))))

(define resolve-chardefs
  (lambda (c) (let ((y (find-chardef c))) (if y (caddr y) #f))))
...
(define tex-def-char
  (lambda (char args expansion frame)
    (unless (and
             (eqv? frame *global-texframe*)
             (or (char=? char #\space) (char=? char #\newline)))
      (cond
       ((assoc char (texframe.chardefinitions frame))
        =>
        (lambda (c) (set-car! (cdr c) args) (set-car! (cddr c) expansion)))
       (else
        (set!texframe.chardefinitions
          frame
          (cons
           (list char args expansion)
           (texframe.chardefinitions frame))))))))

(define tex-def
  (lambda (name args expansion opt-arg frame)
    (cond
     ((lassoc name (texframe.definitions frame) string=?)
      =>
      (lambda (c)
        (set-car! (cdr c) args)
     ...
     (else
      (set!texframe.definitions
        frame
        (cons
         (list name args expansion opt-arg)
...
(define tex-let-prim
  (lambda (cs-new cs-old)
    (cond
     ((lassoc cs-old (texframe.definitions *primitive-texframe*) string=?)
      =>
      (lambda (cs-old-entry)
          ...
          (cond
           ...
           (else
            (set!texframe.definitions
              *primitive-texframe*
              (cons
               (list cs-new cs-old-args cs-old-expn cs-old-optarg)
               (texframe.definitions *primitive-texframe*))))))))
  ...
  (lambda (name thunk frame)
    (cond
     ...
     (else
      (set!texframe.definitions
        frame
        (cons (list name ':thunk thunk #f) (texframe.definitions frame)))))))
...
(define initialize-global-texframe
  (lambda ()
    (tex-def-count "\\language" 256 *global-texframe*)
    (tex-def-count "\\secnumdepth" -2 *global-texframe*)
    (tex-def-count "\\tocdepth" -2 *global-texframe*)
    (tex-def-count "\\footnotenumber" 0 *global-texframe*)
    (tex-def-count "\\TIIPtabularborder" 1 *global-texframe*)
    (tex-def-count "\\TIIPnestedtabularborder" 0 *global-texframe*)
    (tex-def-count "\\TIIPobeyspacestrictly" 0 *global-texframe*)
...
(define find-chardef
  (lambda (char)
    (or (ormap
         (lambda (fr) (assoc char (texframe.chardefinitions fr)))
         *tex-env*)
...
(define find-def
  (lambda (ctlseq)
    (or (ormap
         (lambda (fr) (lassoc ctlseq (texframe.definitions fr) string=?))
         *tex-env*)
        (lassoc ctlseq (texframe.definitions *global-texframe*) string=?)
        (lassoc ctlseq (texframe.definitions *primitive-texframe*) string=?))))
...
(define set-gcount!
  (lambda (ctlseq v) (tex-def-count ctlseq v *global-texframe*)))
...
(define do-the
  (lambda ()
    (let ((ctlseq (get-ctl-seq)))
      (cond
       ((find-count ctlseq) => (lambda (x) (emit (cadr x))))
...
(define find-corresp-prim
  (lambda (ctlseq)
    (let ((y (find-def ctlseq)))
      (if (and y (eqv? (cadr y) ':prim)) (caddr y) ctlseq))))

(define find-corresp-prim-thunk
  (lambda (ctlseq)
    (let ((y (find-def ctlseq)))
      (if (and y (eqv? (cadr y) ':thunk)) (caddr y) ctlseq))))
    ...
    (let* ((c (get-tex-char-spec)) (val (begin (get-equal-sign) (get-number))))
      (case val
        ...
        (else
         (let ((y
                (or (ormap
                     (lambda (fr) (assoc c (texframe.chardefinitions fr)))
                     *tex-env*)
...
(define do-defcsactive
  (lambda ()
    (ignorespaces)
    (let* ((c (get-ctl-seq)) (rhs (ungroup (get-group))) (fr (top-texframe)))
...
(define do-undefcsactive
  (lambda ()
    ...
    (let* ((c (string-ref (get-ctl-seq) 1))
      ...
      (if y (set-car! (cddr y) #f) (tex-def-char c '() #f fr)))))
  ...
  (lambda (global?)
    (let* ((ctlseq (get-ctl-seq))
      ...
      (tex-def
        ctlseq
        '()
        (number->string (+ (string->number (resolve-defs ctlseq)) increment))
        #f
        (if global? *global-texframe* (top-texframe))))))
...
(define do-newenvironment
  (lambda (renew?)
    ...
    (let* ((envname (string-trim-blanks (ungroup (get-token))))
      ...
      (tex-def
        bs-envname
        (latex-arg-num->plain-arg-pat argc)
        beginning
        optarg
        fr)
        ...
        (unless (and
          ...
          (let ((fr (if global? *global-texframe* (top-texframe))))
            (if (ctl-seq? rhs)
              (let ((s (find-def rhs)))
                (if s
                  (tex-def lhs (cadr s) (caddr s) (cadddr s) fr)
  ...
  (lambda (lhs)
    (let aux ()
      (let ((c (snoop-actual-char)))
        (cond
         ((eof-object? c)
          (terror 'read-arg-pat "EOF found while scanning definition of " lhs))
         ((char=? c #\{) '())
         (else (get-actual-char) (cons c (aux))))))))
                      ...
                      (loop
                       ...
                       (cons
                        (if (or (= (+ k 2) arg-pat-n)
                          ...
                          (ungroup (get-token))
                          (get-till-char (list-ref arg-pat (+ k 2))))
                        r)))
                     ((char-whitespace? c) (ignorespaces) (loop (+ k 1) r))
                     (else
    ...
    (let ((in-text? (not (munched-a-newline?))))
      ...
      (fluid-let
        ...
        (let loop ((nesting 0))
          (let ((c (get-actual-char)))
            (cond
             ((eof-object? c) (terror 'do-braced-verb "Eof inside verbatim"))
             ((char=? c *esc-char*)
              (toss-back-char c)
...
(define do-verb
  (lambda ()
    ...
    (fluid-let
      ((*ligatures?* #f))
      ((if (char=? (snoop-actual-char) #\{) do-braced-verb do-latex-verb)))
    (egroup)))
           ...
           (let ((c (get-actual-char)))
             (cond
              ...
              ((char=? c *esc-char-verb*)
               ...
               (loop nesting))
              ((char=? c #\{) (write-char c *verb-port*) (loop (+ nesting 1)))
              ((char=? c #\})
               (unless (= nesting 0)
                 (write-char c *verb-port*)
                 (loop (- nesting 1))))
              (else (write-char c *verb-port*) (loop nesting))))))
        (else
         (let loop ()
           (let ((c (get-actual-char)))
             (cond
              ((eof-object? c)
               (terror 'verb-write-to-file "Eof inside verbatim"))
              ((char=? c d) 'done)
              (else (write-char c *verb-port*) (loop))))))))))
  ...
  (lambda ()
    ...
    (fluid-let
      ...
      (let loop ()
        (let ((c (snoop-actual-char)))
          (cond
           ((eof-object? c) (terror 'do-latex-verbatim "Eof inside verbatim"))
           ((char=? c #\\)
            (let ((end? (get-ctl-seq)))
              (if (string=? end? "\\end")
                (cond
                 ((get-grouped-environment-name-if-any)
                  ...
                  (lambda (e)
                    (unless (string=? e "verbatim")
                      ...
                      (emit-html-char #\})
                      (loop))))
                 (else (for-each emit-html-char end?) (loop)))
...
(define do-scm-set-special-symbol
  (lambda ()
    (let* ((sym (string-trim-blanks (ungroup (get-group)))) (xln (get-group)))
      ...
      (lambda ()
        (let loop ()
          ...
          (unless (eof-object? (snoop-actual-char))
            (table-put! *scm-special-symbols* (scm-get-token) #f)
            (loop)))))))
...
(define scm-set-math-escape
  (lambda (yes?)
    (let ((c
           (fluid-let
             ((*esc-char* (integer->char *int-corresp-to-nul*)))
...
(define scm-output-slatex-comment
  (lambda ()
    (let ((s (get-line)))
      ...
      (when *scm-dribbling?* (display s *scm-port*) (newline *scm-port*))
  ...
  (lambda ()
    (unless *scm-port*
      (let ((output-file (string-append *jobname* ".scm")))
        (ensure-file-deleted output-file)
        (set! *scm-port* (open-output-file output-file))))))
        ...
        ((#\{)
         (let loop ((nesting 0))
           (let ((c (get-actual-char)))
             (cond
              ((eof-object? c)
               (terror 'do-scm-write-to-file "Eof inside verbatim"))
              ((char=? c *esc-char-verb*)
               (write-char c *scm-port*)
               (write-char (get-actual-char) *scm-port*)
               (loop nesting))
              ((char=? c #\{) (write-char c *scm-port*) (loop (+ nesting 1)))
              ((char=? c #\})
               (unless (= nesting 0)
                 (write-char c *scm-port*)
                 (loop (- nesting 1))))
              (else (write-char c *scm-port*) (loop nesting))))))
        (else
         (let loop ()
           (let ((c (get-actual-char)))
             (cond
              ((eof-object? c)
               (terror 'do-scm-write-to-file "Eof inside verbatim"))
              ((char=? c d) 'done)
              (else (write-char c *scm-port*) (loop))))))))))

(define do-scm-slatex-display
  (lambda (env result?)
    (let ((endenv (string-append "\\end" env)))
      (fluid-let
        ...
        (let loop ()
          (let ((c (snoop-actual-char)))
            (cond
             ((eof-object? c)
              (terror 'do-scm-slatex-display "Eof inside {schemedisplay}"))
             ((char=? c *esc-char*)
              (let ((x (get-ctl-seq)))
                (cond
                 ((string=? x endenv) #t)
...
(define eat-star
  (lambda ()
    (let ((c (snoop-actual-char)))
      (when (and (not (eof-object? c)) (char=? c #\*)) (get-actual-char)))))
  ...
  (lambda (z)
    ...
    (let ((top-tabular
      ...
      (case top-tabular
        ...
        ((ruled-table) (emit "</td></tr><tr><td>") (emit-newline))
        ((minipage tabbing)
         (get-bracketed-text-if-any)
         (emit "<br>")
         (emit-newline))
...
(define set-latex-counter
  (lambda (add?)
    (let* ((counter-name (string-trim-blanks (ungroup (get-group))))
           (new-value
             (string->number (string-trim-blanks (ungroup (get-group))))))
      (cond
...
(define do-tex-ctl-seq-completely
  (lambda (x)
    (let ((z (resolve-defs x)))
      (cond
       ((not (eq? z x)) (tex2page-string z))
       ((do-tex-prim (find-corresp-prim z))
        =>
        (lambda (y)
          (if (eqv? y ':encountered-undefined-command) (emit x))))))))
...
(define do-tex-ctl-seq
  (lambda (x)
    ...
    (let ((z (resolve-defs x)))
      (cond
       ((not (eq? z x))
        (trace-if *tracingmacros?* "    --> " z)
        (toss-back-char *invisible-space*)
        (toss-back-string z))
       ((and *doing-urlh?* (string=? z "\\\\")) (egroup))
...
(define do-if-file-exists
  (lambda ()
    (let* ((file (ungroup (get-group)))
      ...
      (toss-back-string (if (file-exists? file) thene elsee)))))
...
(define do-input-if-file-exists
  (lambda ()
    (let* ((f (ungroup (get-group)))
      ...
      (tex2page-string
        (if (do-input-aux (actual-tex-filename f)) then-txt else-txt)))))
    ...
    (let ((f
      ...
      (let ((boilerplate-index *inputting-boilerplate?*))
        ...
        (fluid-let
          ...
          (cond
           ((and (or (string=? f "texinfo") (string=? f "texinfo.tex"))
                 (not (eqv? *tex-format* 'texinfo)))
            (set! *tex-format* 'texinfo)
            (unless (find-def "\\ifinfo")
              (let ((texinfo-t2p (actual-tex-filename "texinfo.t2p")))
                (unless (file-exists? texinfo-t2p)
                  (terror 'do-input "Put texinfo.t2p in TIIPINPUTS"))
           ...
           ((or (latex-style-file? f)
            ...
            #f)
           (else (do-input-check-bye (actual-tex-filename f)))))))))

(define do-input-check-bye
  (lambda (f)
    (let ((r (do-input-aux f)))
      (when (and (eqv? f *main-tex-file*) (not (eqv? r ':encountered-bye)))
            ...
            ((file-exists? f)
             (let ((t2p-file?
               ...
               (unless t2p-file?
                 ...
                 (when (eqv? *tex-format* 'texinfo) (set! f (texi-to-tex f))))
               (tex2page-file f)))
            (else (write-log #\space) (write-log "not found") #f))))
      (write-log #\))
      r)))
        ...
        (let ((c (snoop-actual-char)))
          (cond
           ...
           ((ormap (lambda (d) (char=? c d)) *filename-delims*)
            (terror 'do-includeonly))
           (else
            (set! *includeonly-list* (cons (get-filename) *includeonly-list*))
            (loop))))))))
...
(define texi-to-tex
  (lambda (f)
    ...
    (write-log #\{)
    (write-log f)
    (call-with-input-file/buffered f (lambda () (untexinfo-rest-of-doc)))))
                  ...
                  (cond
                   ((char=? c #\@)
                    (let ((x (get-ctl-seq)))
                      (display x o)
                      (cond
                       ((string=? x "\\tex") (dump-till-end-tex o))
                   ...
                   ((ormap (lambda (x) (char=? c x)) '(#\\ #\% #\$ #\#))
                    ...
                    (write-char c o)
                    (loop))
                   (else (get-actual-char) (write-char c o) (loop)))))))))
      (write-log #\})
      fo)))
...
(define dump-till-end-tex
  (lambda (o)
    (let loop ()
      (let ((c (snoop-actual-char)))
        (cond
         ((eof-object? c) (terror 'dump-till-end-tex))
         ((char=? c #\@)
          (let ((x (get-ctl-seq)))
            (cond
             ((string=? x "\\end")
              ...
              (set! x (get-ctl-seq))
              (display x o)
              (unless (string=? x "\\endtex") (loop)))
             ((string=? x "\\comment")
              (write-char #\% o)
              (newline o)
              (eat-till-eol)
              (loop))
             (else (display x o) (loop)))))
         (else (get-actual-char) (write-char c o) (loop)))))))
...
(define do-evalh
  (lambda (s)
    (call-with-input-string
      ...
      (lambda (i)
        (let loop ()
          (let ((x (read i))) (unless (eof-object? x) (eval1 x) (loop))))))))
...
(define do-evalt
  (lambda (i)
    ...
    (let ((scratch-tex-file (string-append *eval-file-stem* ".tex")))
      (fluid-let
        ((*after-eval* #f))
        (with-output-to-file scratch-tex-file (lambda () (do-evalh i)))
...
(define do-eval
  (lambda (fmts)
    ...
    (let ((scheme-expr (ungroup (get-group))))
      (unless (inside-false-world?)
        ((case fmts ((html) do-evalh) ((both) do-evalt)) scheme-expr)))))
  ...
  (lambda (f)
    (when (and
      ...
      (case *operating-system*
        ...
        ((windows)
         (system (string-append *shell-cmd-prefix* "copy " f ".* " *aux-dir*))
         (when (or
                (file-exists? (string-append f ".tex"))
                (file-exists? (string-append f ".scm")))
           (system (string-append *shell-cmd-prefix* "del " f ".*"))))))))
...
(define !using-external-program
  (lambda (x) (set! *external-programs* (cons x *external-programs*))))
    ...
    (let ((n (string-length f)))
      (if (= n 0)
        ...
        (let ((c0 (string-ref f 0)))
          (if (char=? c0 #\/)
            #t
            (if (or (= n 1) (not (eqv? *operating-system* 'windows)))
              #f
              (if (and (char-alphabetic? c0) (char=? (string-ref f 1) #\:))
                #t
                #f))))))))
      ...
      (let ((real-f
        ...
        (when (and
          ...
          (case *operating-system*
            ...
            ((windows)
             (system
               (string-append *shell-cmd-prefix* "copy/b " f " " real-f)))))
        real-f)
      f)))
  ...
  (lambda (cs th)
    (set!texframe.definitions
      *math-primitive-texframe*
      (cons (cons cs th) (texframe.definitions *math-primitive-texframe*)))))
...
(tex-def-prim
  "\\indexsubsubitem"
  (lambda () (do-index-item "\\indexsubsubitem")))
...
(tex-def-prim
  "\\schemedisplay"
  (lambda () (do-scm-slatex-display "schemedisplay" #f)))

(tex-def-prim "\\schemebox" (lambda () (do-scm-slatex-display "schemebox" #f)))

(tex-def-prim
  "\\schemeresponse"
  (lambda () (do-scm-slatex-display "schemeresponse" 'result)))
...
(define tex2page
  (lambda (tex-file)
    (fluid-let
      ...
      (write-log #\newline)
      (cond
       ((not (file-exists? *main-tex-file*)) (tex2page-help tex-file))
       (else
        ...
        (set! *html-page*
          (let ((ext (file-extension *main-tex-file*)))
            (string-append
              *aux-dir/*
              *jobname*
              (if (and ext (string-ci=? ext ".html") (not *aux-dir*))
                "-SAVE.html"
                ".html"))))
        (ensure-file-deleted *html-page*)
        (set! *html* (open-output-file *html-page*))
        ...
        (do-start)
        (let ((t2p-file
                (actual-tex-filename (string-append *jobname* ".t2p"))))

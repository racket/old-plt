...
;            [body (listof string?)])]
    (struct response/incremental
            ([generator ((() (listof string?) . ->* . any) . -> . any)]))
...
; two-digits : num -> str
  ...
  (define (xexpr-exn->response exn x)
    ...
    (pretty-print-size-hook (lambda (v display? op)
                              (string-length (format "~a" v))))
    (pretty-print-print-hook (lambda (v display? op)
                               ((if display? display write)
                                (pretty-format-invalid-xexpr
                                  v (exn:invalid-xexpr-code exn))
                                op)))
    ...
    (make-response/full
      ...
      (list
        (string-append
          ...
          "<pre>" (pregexp-replace "<" (exn-message exn) "&lt;") "</pre>"
  ...
  (define (pretty-format-invalid-xexpr v err)
    (cond
      ((equal? v err)
       (string-append
         "<span style='font-weight: bold; "
         "color: red; background-color: white;'>"
         (pregexp-replace "<" (format "~a" v) "&lt;")
         "</span>"))
      ((list? v)
       (string-append
         "("
         (string-join (map 
                        (lambda (sub-v)
                          (pretty-format-invalid-xexpr sub-v err))
                        v)
                      " ")
         ")"))
      ((string? v) (format "~s" v))
      (else (pregexp-replace "<" (format "~a" v) "&lt;"))))
  |#

;; check out save as... It seems that the frame halfway leaves the project
;; when save as.. happens
	       ...
	       (send* language-menu
		      (append-item "Select Library..."
				   (let ([lib-dir "/home/comp210"]
					 [save-dir ""])
				     (lambda ()
				       (set! save-dir (mred:current-find-file-directory))
				       (mred:current-find-file-directory lib-dir)
				       (let ([lib-file (mred:get-file () "Select a Library" ".*\\.ss$")])
					 (when lib-file
					   (mred:set-preference 'drscheme:library-file lib-file)))
				       (set! lib-dir (mred:current-find-file-directory))

...
(let ([p (open-input-string "a\nb")])
  ...
  (port-count-lines! p)
  (test '(1 1 1) call-with-values (lambda () (port-next-location p)) list)
  (test #\a read-char p)
  (test '(1 2 2) call-with-values (lambda () (port-next-location p)) list)
  (test '(1 2 2) call-with-values (lambda () (port-next-location p)) list)
  (test #\newline peek-char p)
  (test '(1 2 2) call-with-values (lambda () (port-next-location p)) list)
  (test #\newline read-char p)
  (test '(2 1 3) call-with-values (lambda () (port-next-location p)) list)
  (test #\b peek-char p)
  (test '(2 1 3) call-with-values (lambda () (port-next-location p)) list)
  (test #\b read-char p)
  (test '(2 2 4) call-with-values (lambda () (port-next-location p)) list)

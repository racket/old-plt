  ...
  (define-syntax define-struct
	...
	(let ([syntax-error
	  ...
	  (for-each (lambda (x) 
		    ...
		    (stx->list (cadr body)))
	  (or (null? (cddr body))
	      (let ([inspect (stx->list (caddr body))])
		(if (not (and (stx-list? inspect)
			      (<= 2 (length inspect) 3)))
		    (syntax-error "thrid part must be an inspector expression or inspector-expression/opacity-expression pair"))))
	  (let ([name (if (identifier? (car body))
		...
		[inspector (if (null? (cddr body))
			       #f
			       (stx-car (caddr body)))]
		[opaque? (if (or (null? (cddr body))
				 (null? (stx-cdr (caddr body))))
			     #t
			     (stx-car (stx-cdr (caddr body))))])
	    (datum->syntax
	     `(define-values

	...
	(class-asi super%
	  (inherit keymap)
		   ...
		   (public
		    ...
		    [begin-list  '("cond" "begin" "begin0" "delay"
					  "public" "private"
		    ...
		    [tabify    
			 ...
			 (letrec	
				 ...
				 (let* ([proc-name (get-proc)]
					[same? (lambda (str)(string=? str proc-name))])
				   (cond [(ormap same? define-list) 1]
					 [(ormap same? begin-list)  1]
					 [(ormap same? lambda-list) 3]
					 [else 0])))]
			      [special-check
			       (lambda ()
				 (let* ([proc-name (get-proc)]
					[same? (lambda (str)(string=? str proc-name))])
				   (or (ormap same? define-list)
				       (ormap same? lambda-list))))]
			      [indent-first-arg
			       (lambda (start)

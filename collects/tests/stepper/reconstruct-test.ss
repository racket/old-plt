  ...
  (let/ec k
    (let* ([action (lambda (before after)
                     (when (null? expected-queue)
                       (test #f (lambda () expected-completed))
                       (k (void)))
                     (test (car expected-queue) (lambda () before))
                     (test (cadr expected-queue) (lambda () after))
                     (set! expected-queue (cddr expected-queue)))])
      (parameterize ([current-namespace namespace])
        (let ([annotated (annotate-exprs stx action)])
          (if expected-completed
              (test expected-completed map reconstruct:reconstruct-completed annotated (map eval annotated))
              (map eval annotated))

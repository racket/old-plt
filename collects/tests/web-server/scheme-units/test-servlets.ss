...
(module test-servlets mzscheme
  ...
  (require/expose (lib "web-server-unit.ss" "web-server") (get-host DEFAULT-HOST-NAME))
  ...
  (define test-servlets
    (make-test-suite
     ...
     (make-test-suite
      "Test Cases Related to PR#7192"

      (make-test-case
       "Original Test Case for PR#7192"
       (let ([stop-server (start-server)])
         (let-values ([(ip op) (tcp-connect "localhost" THE-PORT)])
           (fprintf op "GET http://www.google.com/ha HTTP/1.0~n~n")
           (let ([resp (read-line ip)])
             (printf "resp = ~s~n"  resp)
             (begin0
               (assert-true
                (and (input-port? ip) (output-port? op)
                     (not (eof-object? resp))))
               (stop-server))))))

      (make-test-case
       "get-host w/host in url but not in headers"
       (let ([result
              (get-host (string->url "http://www.google.com/ha") '())])
         (assert-true (string=? result "www.google.com"))))

      (make-test-case
       "get-host w/host in url, not in headers, host has uppercase chars"
       (let ([result
              (get-host (string->url "http://www.GooGLE.com/ha") '())])
         (assert-true (string=? result "www.google.com"))))

      (make-test-case
       "get-host w/out host in url nor in headers"
       (let ([result
              (get-host (string->url "foo") '())])
         (assert-true (string=? result DEFAULT-HOST-NAME))))

      (make-test-case
       "get-host w/ host in headers but not in url"
       (let ([result
              (get-host (string->url "foo") (list '(host . #"my-host")))])
         (assert-true (string=? result "my-host"))))

      (make-test-case
       "get-host w/ host in headers but not in url, host has uppercase chars"
       (let ([result
              (get-host (string->url "foo") (list '(host . #"my-HOST")))])
         (assert-true (string=? result "my-host")))))

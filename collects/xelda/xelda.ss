(module xelda mzscheme
  ...
  (define (distinguish-circular-formulas)
  ...
  (define (compute-formulas-circular)
    ...
    (printf "Checking non-circular formulas.... ")
    (update-status! "Checking non-circular formulas.... ")
    (compute-formulas)
    (printf "done~nAssigning dimension variables for formulas.... ")
    (next-update-status! "Assigning dimension variables for formulas");
    (assign-variables)
      ...
      [($ application name deps fun args) 
       (let ([arg-syms (map (lambda (a)
         ...
         (case fun
           ...
           [(npv)
            (cond [(>= (length args) 2)
                   (push-constraint (list '= (first arg-syms) (empty-unit)))
                          ...
                          (if (andmap (lambda (u) (integer? (/ (second u) new-exp)))
                              ...
                              (let ([u (map
                                ...
                                (when (not (in-hash? hashed-constraints-vars new-var))
                                  (hash-table-put! hashed-constraints-vars new-var u))

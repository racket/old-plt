(module xelda mzscheme
  ...
  (define (replace-in-constraints sym1 sym2)
    (let* ([replace (lambda (l) 
                      (map (lambda (s)
                             (cond [(eq? s sym1) sym2]
                                   [else s]))
                           l))]
      ...
      (set! constraints 
            (list
             (remove-duplicates
              (filter (lambda (c) (not (eq? (second c) (third c))))
  ...
  (define (add-to-constraints-vars sym1 sym2)
    (cond [(in-hash? hashed-constraints-vars sym1)
           (hash-table-put!
            hashed-constraints-vars sym1
            (cons sym2 (hash-table-get hashed-constraints-vars sym1)))]
          [else 
           (hash-table-put! hashed-constraints-vars 
                            sym1 
                            (list sym2))]))
  
  (define (replace-in-constraints-vars sym1 sym2)
    (hash-table-for-each 
    ...
    (letrec ([to-dims 
      ...
      (filter 
       ...
       (map 
        (lambda (c)
          (if (= 3 (length c))
              (if (eq? (first (third c)) 'empty-unit)
              ...
              (cons 
               ...
               (cons
                ...
                (filter
                 (lambda (u)
                   (not (eq? (first u) 'empty-unit))) (rest (rest c)))))))
        (map 
         (lambda (c)
           ...
           (when (not (not (assq var right-side)))
             (let ([var_exp (- (second (assq var right-side)) 1)]
                   [filtered-right-side (filter (lambda (u)
                                                  (not (eq? (first u) var)))
                                                right-side)])
               (cond [(= 0 var_exp) (add-to-constraints-vars var (list (list 'error/infinity 1)))]
                     ...
                     [(and (andmap (lambda (u) (not (dim-var? (first u))))
                      ...
                      (let ([u (map
                                ...
                                filtered-right-side)])
                        (when (not (in-hash? hashed-constraints-vars var))
                          (hash-table-put! 1st-pass-values var u))
                        (add-to-constraints-vars var u))]
                     [else (add-to-constraints-vars var (list (list 'error/not-empty 1)))])))))
       cs)
       ...
       (let ([right-side (rest (rest c))])
         (cond
           [(ormap (lambda (u) (dim-var? (first u))) right-side)
            (add-to-constraints-vars (first c) (list (list 'error/infinity 1)))
            (for-each
             (lambda (u)
               (when (dim-var? (first u))
                 (add-to-constraints-vars (first u) (list (list 'error/infinity 1)))))
             right-side)]
           [else
            (add-to-constraints-vars (first c) (simplify right-side))])))
     cs)
    empty)
    ...
    (let* ([vals (cond [(in-hash? hashed-constraints-vars var)
           ...
           [dims-only 
            (append  dims
                     (map 
                      (lambda (var)
                        (resolve-constraints-var var)
                        (first (hash-table-get hashed-constraints-vars var)))
                      vars))]
           [errs-actual-dims (split-list dims-only is-error-unit?)]
           [errs (filter (lambda (e)
                           (not (eq? (first (first e)) 'error/infinity)))
                         (first errs-actual-dims))]
           [actual-dims (second errs-actual-dims)])
      (cond
        [(not (empty? errs)) (hash-table-put! hashed-constraints-vars
                                              var
                                              (list (first errs)))]
        ...
        [(= 1 (length actual-dims)) (hash-table-put! hashed-constraints-vars
                                                     var
                                                     actual-dims)]
        [else (hash-table-put! hashed-constraints-vars
                               var
                               (list (check-equal-units actual-dims var)))])))
     ...
     (lambda (cell unit)
       (if (is-error-unit? unit)
           (let ([cell-rng (cellref->rng cell)]
             ...
             (set-range-color! cell-rng (rgb 250 200 80))
       ...
       (let* ([cell (car annot)]
         ...
         (if (not (or (in-hash? error-cells cell)
             ...
             (let ([cell-rng (cellref->rng cell)]
               ...
               (fprintf o "MISMATCH computed:~n~a " actual)
               (fprintf o "cell-formula: ~a~n" (lookup-formula-text cell))
               (insert-comment cell-rng (get-output-string o))
               (hash-table-put! error-cells cell 'on)
               (update-status! 
  ...
  (define (get-units-from-txt str)
    (let ([unit
            (let ([p (open-input-string str)])
              (with-handlers ([void (lambda _ '())])
                (read p)))])
      (cond [(or (empty? unit) (dim? unit)) unit]
            [else '()])))

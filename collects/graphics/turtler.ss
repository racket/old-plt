...
;; by a turtle -- but it is a delta not an absolute.
    ...
    (define Window #f)
    (define init-error (lambda _ (error 'turtles "Window is not shown. eval: (turtles #t)")))
    (define line init-error)
    (define wipe-line init-error)
    (define clear-window init-error)
    (define show init-error)

    (define turtles
      (let ([state initially-shown?])
	(case-lambda
	 [() (turtles (not state))]
	 [(x)
	  (set! state x)
	  (unless Window
	    (set! Window (make-object plot-window% "Turtles"
				      turtle-window-size
				      turtle-window-size))
	    (set! line (ivar Window draw-line))
	    (set! wipe-line (ivar Window wipe-line))
	    (set! clear-window (ivar Window clear))
	    (set! show (ivar Window show)))
	  (show x)])))

...
#ifdef __GNUG__
#pragma implementation "wxregex.h"
#include <std.h>
//#include <builtin.h>
#endif
...
#   include "wxregex.h"
#else
    // Xt port uses another directory structure
...
const wxRegex RXwhite("[ \n\t\r\v\f]+", 1);
const wxRegex RXint("-?[0-9]+", 1);
const wxRegex RXdouble("-?\\(\\([0-9]+\\.[0-9]*\\)\\|\\([0-9]+\\)\\|\\(\\.[0-9]+\\)\\)\\([eE][---+]?[0-9]+\\)?", 1, 200);
const wxRegex RXalpha("[A-Za-z]+", 1);
const wxRegex RXlowercase("[a-z]+", 1);
const wxRegex RXuppercase("[A-Z]+", 1);
const wxRegex RXalphanum("[0-9A-Za-z]+", 1);
const wxRegex RXidentifier("[A-Za-z_][A-Za-z0-9_]*", 1);
...
#else
#if defined(sparc) || defined(__sgi) || defined(__DECCXX)
#include <alloca.h>

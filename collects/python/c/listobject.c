...
#define NRESIZE(var, type, nitems) nresize(var, nitems)
#endif
...
#ifdef DEBUG_INS
 #define INSF(fmt, args...) printf(fmt, ##args)
#else
 #define INSF(fmt, args...)
#endif
...
ins1(PyListObject *self, int where, PyObject *v)
{
  ...
  if ( doprint1 ) { doprint1 = 0;
 INSF("ins1: BEFORE: ");
 INSF("%s", PyString_AsString(PyList_Type.tp_repr(self)));
 INSF("\n"); doprint1 = 0; }
	items = self->ob_item;
  INSF("ins1: inserting value: %s\n",
   SCHEME_STR_VAL((Scheme_Object*)sapply1("py-object%->string", v)));
   INSF("ins1: value type: %s\n",
   SCHEME_STR_VAL((Scheme_Object*)sapply1("py-object%->string",PY_GET_TYPE(v))));
   INSF("ins1: value address: 0x%x\n", v);
INSF("ins1: before ins1 resize\n");
INSF("ins1: self->ob_size = %d, where = %d\n", self->ob_size, where);
INSF("ins1: items: 0x%x\n", items);
#ifdef DEBUG_INS
if ( items )
  {
  int idx;
  for ( idx = 0; idx < self->ob_size; idx++ )
    INSF("ins1:  items[%d]: 0x%x\n", idx, items[idx]);
  }
else
  INSF("ins1: cannot deref items (it's null)\n");
#endif

	NRESIZE(items, PyObject *, self->ob_size+1);

INSF("ins1: after ins1 resize\n");
	if (items == NULL) {
		PyErr_NoMemory();
		return -1;
	}
 ...
 if ( items )
  {
  int idx;
  INSF("ins1: printing new items\n");
  for ( idx = 0; idx < self->ob_size; idx++ )
    {
    PyObject* item = items[idx];
    INSF("ins1:  items[%d]: 0x%x\n", idx, item);
    INSF("ins1:  value: %s\n",
           SCHEME_STR_VAL((Scheme_Object*)sapply1("py-object%->string", item)));
    }
  }
else
  INSF("ins1: cannot deref items (it's null)\n");
...
{
INSF("ins: ins called. is self a list? %d, is v? %d\n",
       self->ob_type == &PyList_Type, v->ob_type == &PyList_Type);
...
listsort(PyListObject *self, PyObject *args)
{
	...
	printf("listsort ENTERED\n");
	assert(self != NULL);
	if (args != NULL) {
	  printf("listsort: args are not null\n");
		if (!PyArg_UnpackTuple(args, "sort", 0, 1, &compare))
			return NULL;
	}
	else printf("listsort: null args\n");
	if (compare == Py_None)
	  printf("listsort: compare is None\n");
	else if (compare == NULL)
	  printf("listsort: compare is null\n");
	else printf("listsort: compare is ok\n");
	...
	printf("listsort: saved ob size = %d\n", saved_ob_size);
	printf("listsort: nremaining = %d\n", nremaining);
	if (nremaining < 2)
		goto succeed;
	...
	do {
		...
		printf("listsort: run n = %d\n", n);
		printf("listsort: descending = %d\n", descending);
		if (n < 0)
			goto fail;
		if (descending)

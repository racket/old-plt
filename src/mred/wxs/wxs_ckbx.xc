...
@SET CALLBACK_CLASS_USER = "check-box%::initialization"
...
@CREATOR (wxPanel!,wxFunction/bCallback/ubCallback/cCallback//spCallback,wxBitmap!,int=-1,int=-1,int=-1,int=-1,SYM[checkboxStyle]=0,string="checkBox"); : : ubCallbackSetup/CHECKOK[2."check-box%::initialization"]|NOZERO[5]|NOZERO[6]//ubCallbackCreatorFinish <> bitmap label

@INCLUDE wxs_item.xci

@ "get-value" : bool GetValue()
@ "set-value" : void SetValue(bool);
@ "set-label" : void SetLabel(wxBitmap!); : : /CHECKOK[0."check-box%::set-label"] <> bitmap label

...
;  x-posn/y-posn: numbers which indicate the default position of the child.
...
;   two results.
; input: function: a function which takes two boxes and returns results in
;   them.
; returns: the contents of the two boxes (as multiple values)
(define mred:get-two-int-results
  (lambda (function)
    (let ([a (box 0)]
	  [b (box 0)])
      (function a b)
...
; mred:make-item%: creates items which are suitable for placing into
;  containers.
; input: item%: a wx:item% descendant (but see below) from which the new
;                 class will be derived.
;        stretch-x?/stretch-y?: booleans which specify the default
;          stretchability behavior for the new class.
;        make-default-size: a function which takes the args of the item%
;          class and returns a list of these args but ensures that the size
;          args are both -1 (default size).
; returns: a class, descended from wx:item%, which is suitable for placing
;            in a container.
; Note: the item% parameter does not necessarily HAVE to be a descendant of
; wx:item%, so long as it contains the identifiers in the inherit section
; below.  You will note below that I ran wx:panel% through this function to
; create mred:container-panel%.  (Yes, this is cheating.  So what's your
; point?)
(define mred:make-item%
  (lambda (item% stretch-x? stretch-y? make-default-size)
	...
	; store minimum size of item.  Since we no longer support label
        ; changes, this will never change, so we don't have to maintain a
        ; validity indicator.
	min-width
	min-height

	; the "default position" of the object; defined to be the creation
	...
	; returns: (if nothing passed in) current stretchability property.
	...
	; stretchable-in-y?: see stretchable-in-x? but substitute "y" for
        ;   "x" and "horizontal" for "vertical".
	[stretchable-in-y?
	  (make-stretchable-parameter stretch-y? 'stretchable-in-y?)]

	; get-info: passes necessary info up to parent.
	; input: none
	; returns: mred:child-info struct containing the info about this
        ;   item.
	...
	[get-info
	  (lambda ()
	    ...
	    (let* ([min-size (get-min-size)]
		   [result (make-mred:child-info default-x default-y
	...
	; get-min-size: computes the minimum size the item can reasonably
        ;   assume.
	; input: none
...
; The various classes that we pass through this function do not all have
; their parameter lists in the same order, so the position and size
; args tend to move around.  There are two alternatives: either change the
; parameter lists so that they are more consistent, or the method used
; here.  The first solution would make modifying existing code to use the
; mred:item% classes significantly more difficult, as the parameter lists
; would also have to be reordered.
...
; returns: a list of the parameters with mred:const-default-size
;   substituted in for w and h.
; Note: this function works with wx:button%, wx:check-box%, wx:choice%, and
;   wx:radio-box%.
(define standard-make-default-size
  (opt-lambda (parent callback label
		[x mred:const-default-posn]
		[y mred:const-default-posn]
		[w mred:const-default-size]
		[h mred:const-default-size] . args)
    `(,parent ,callback ,label ,x ,y ,mred:const-default-size
       ,mred:const-default-size ,@args)))

; these next definitions descend classes from the children of wx:item%
; which can be inserted into mred:container-panel% objects.
(define mred:button%
  (mred:make-item% wx:button% #t #t standard-make-default-size))

(define mred:check-box%
  (mred:make-item% wx:check-box% #f #f standard-make-default-size))

(define mred:choice%
  (mred:make-item% wx:choice% #t #f standard-make-default-size))

;(define mred:gauge%
;  (mred:make-item% wx:gauge% #t #f

(define mred:group-box%
  "Exactly what are we going to do with these things?")

(define mred:list-box%
  (mred:make-item% wx:list-box% #t #t
    (opt-lambda (parent callback label
	 ...
	 ,mred:const-default-size ,mred:const-default-size
	 ,@args))))

(define mred:message%
  (mred:make-item% wx:message% #t #t list))
; we don't need to process the size args at all cause there aren't any.
; Therefore, we just need to bundle the args up in a list.  list already
; does that.

(define mred:radio-box%
  (mred:make-item% wx:radio-box% #f #f standard-make-default-size))

(define mred:slider%
  (mred:make-item% wx:slider% #t #f list))

(define mred:text-window%
  (mred:make-item% wx:text-window% #t #t
    (opt-lambda (parent
		  [x mred:const-default-posn]
		  [y mred:const-default-posn]
		  [w mred:const-default-size]
		  [h mred:const-default-size] . args)
      `(,parent ,x ,y ,mred:const-default-size ,mred:const-default-size
	 ,@args))))

(define mred:text%  ;; for now
  (mred:make-item% wx:text% #t #f
    (opt-lambda (parent callback label
		  [val ""]
		  [x mred:const-default-posn]
		  [y mred:const-default-posn]
...
(define mred:multi-text%
  (mred:make-item% wx:multi-text% #t #t
    (opt-lambda (parent callback label
		  [val ""]
		  [x mred:const-default-posn]
		  [y mred:const-default-posn]
...
(define mred:intermediate-panel%
  (mred:make-item% wx:panel% #t #t list))

; mred:container-panel%: a class with all the functionality of a wx:panel%
; that can hold mred:items and reposition them as necessary.  Note that a
; panel can contain other panels.
(define mred:container-panel%
  (class mred:intermediate-panel% args
    (inherit
      object-ID
      get-width
      get-height
    ...
    (public
      ...
      ; list of mred:child-info structs corresponding to the children.  (#f
      ;  if no longer valid.)
      [children-info null]

      ; cache to prevent on-size from recomputing its result every time.
      ; when curr-width is #f, cache invalid.
      curr-width
      curr-height

      ; add-child: adds an existing child to the panel.
      ; input: new-child: mred:item% descendant to add
      ;        adder-function: a function which takes a mred:item% and a
      ;          list of mred:item%s and returns a list of mred:item%s.
      ; returns: nothing
	...
	(lambda (child)
      ...
      ; get-children-info: returns children info list, recomputing it if
      ;   needed.
      ; input: none
      ; returns: list of mred:child-info structs.
	...
	(lambda ()
	  ...
	  (unless children-info
	    ...
	    (set! children-info (map (lambda (child) (send child get-info))
				  children)))
	  children-info)]
      ...
      ; input: none
      ; returns: nothing
      ; effects: sends a message up to the top container to redraw itself
	...
	(lambda ()
	  ...
	  (let ([parent (get-parent)])
		...
		(let-values ([(width height)
			      (mred:get-two-int-results get-client-size)])
		  (redraw width height))
		(send parent force-redraw))))]

      ; get-min-size: poll children and return minimum possible size for
      ;   the container.
      ; input: none
      ; returns: minimum size (as a list, width & height) of container.
      ; effects: none.
      [get-min-size
	(letrec ([gms-helper
		   (lambda (kid-info x-accum y-accum)
			 ...
			 (let ([curr-info (car kid-info)])
			   (gms-helper
			     (cdr kid-info)
			     (max x-accum
			       (+ mred:default-spacing
				  (mred:child-info-x-posn curr-info)
				  (mred:child-info-x-min curr-info)))
			     (max y-accum
			       (+ mred:default-spacing
				  (mred:child-info-y-posn curr-info)
				  (mred:child-info-y-min
				    curr-info)))))))])
	  (lambda ()
	    (gms-helper (get-children-info)
	      mred:default-spacing mred:default-spacing)))]

      ; on-size: called when the container is resized (usu by its parent)
	...
	(letrec ([redraw-helper
		   (lambda (children children-info)
			 ...
			 (send curr-child set-size
			   ...
			   (mred:child-info-y-posn curr-info)
			   (mred:child-info-x-min curr-info)
			   (mred:child-info-y-min curr-info))
...
(define make-top-container%
  (lambda (base%)
	...
	; input: new-panel: panel in frame (descendant of
        ;          mred:container-panel%) 
	; returns: nothing
	; effects: sets panel to new-panel
	;          if new-panel is not a descendant of mred:container-panel%,
	;          calls error; panel not updated.
	[insert-panel
	  (lambda (new-panel)
	    (if (is-a? new-panel mred:container-panel%)
		(set! panel new-panel)
		(error 'insert-panel
		  "Expected a mred:container-panel% descendant; got ~s"
		  new-panel)))]
	...
	; returns: nothing
	; effects: if new size is smaller than allowed size of contents,
	;            frame resized to smallest possible size.  If frame is
	;            larger than contents and contents aren't stretchable,
	;            frame resized to size of contents.  Each direction is
	;            handled independantly.
	[on-size
	  (lambda (new-width new-height)
	    (printf "FRAME: Entered frame's on-size; args ~s ~s~n" new-width
	      new-height)
	    (unless (null? panel)
	      (let ([p-x (box 0)]
		    [p-y (box 0)])
		...
		(let* ([panel-info (send panel get-info)]
		       ...
		       [min-width (mred:child-info-x-min panel-info)]
		       [min-height (mred:child-info-y-min panel-info)]
		       
		       ; panel's client size
		       [p-client-width (unbox p-x)]
		       [p-client-height (unbox p-y)]
		       
		       ; difference between panel's client & full sizes.
		       ...
		       [new-w (cond
				[(< p-client-width min-width) min-width]
				[(and (> p-client-width min-width)
				   (not (mred:child-info-x-stretch
					panel-info)))
				 min-width]
				[else p-client-width])]
		       [new-h (cond
				[(< p-client-height min-height) min-height]
				[(and (> p-client-height min-height)
				   (not (mred:child-info-y-stretch
					  panel-info)))
				 min-height]
				[else p-client-height])]
		       
		       ; new frame size, computed from above quantities.
...
(define mred:container-frame% (make-top-container% wx:frame%))
(define mred:container-dialog% (make-top-container% wx:dialog-box%))
      

; make-get-size: creates a function which returns the minimum possible size
;   for a mred:horizontal-panel% or mred:vertical-panel% object.
; input: container: a pointer to the mred:container-panel% descendant upon
;          which this function operates.
;        compute-x/compute-y: functions which take the current x/y
;          location, the inter-object spacing, and the mred:child-info
;          struct corresponding to the current object, and return the new
;          x/y locations.
;        spacing: (defaults to mred:default-spacing).  The size of the gap
;        between adjacent objects and objects and the edge of the panel.
...
(define make-get-size
  (opt-lambda (container compute-x compute-y
	...
	(printf "Entering get-min-size; object ~s~n" (ivar container
						       object-ID))
	(gs-help (send container get-children-info) spacing spacing)))))

; make-h-v-redraw: creates functions suitable for use as the redraw
; function in a mred:horizontal-panel% or mred:vertical-panel% class.
; input: container: pointer to the mred:container-panel% object which uses the
;          resulting function.
;        child-major-size: function which takes a mred:child-info struct
;          and returns the child's minimum size in the major direction of the
;          panel.
;        child-major-stretch: function which takes a mred;child-info struct
;          and returns the child's stretchability in the major direction of
;          the panel.
;        child-minor-size/child-minor-stretch: see above.
;        major-dim/minor-dim: functions which take the width and the height
;          of the panel and return the panel's major and minor dimensions,
;          respectively.
;        get-h-info/get-v-info: functions which take info lists describing
;          the major and minor directions and select the appropriate one.
;        spacing: the size of the gaps between adjacent objects and objects
;          and the edge of the panel.
; returns: a function which takes the width and the height of the panel and
;   resizes and moves its children appropriately: centered or stretched
;   along the minor dimension, and equally spaced along the major
;   dimension.
  ...
  (opt-lambda (container child-major-size
	...
	(let* ([get-minor-info
		 (letrec
	       ...
	       [extra-space (compute-extra-space (major-dim width height)
			      kid-info)]
	  ...
	  (let ([major-info (get-major-info kid-info)]
	    ...
	    (printf "# stretchable: ~s;  Extra space: ~s;  " num-stretchable
	      extra-space)
...
; mred:horizontal-panel%: a panel which arranges its children in an evenly
; spaced horizontal row.  Items are vertically centered (or stretched to
; fit the dialog box if they are stretchable).  The items are evenly spaced
; horizontally, with any extra space divided evenly among the stretchable
; items. 
(define mred:horizontal-panel%
  (class-asi mred:container-panel%
    (public
      [get-min-size
	(make-get-size this
	  (lambda (curr-x spacing curr-info)
	    (+ curr-x spacing (mred:child-info-x-min curr-info)))
	...
	(make-h-v-redraw this
	  mred:child-info-x-min
	  mred:child-info-x-stretch
	  mred:child-info-y-min
	  mred:child-info-y-stretch
	  (lambda (width height) width)
	  (lambda (width height) height)
	  (lambda (major minor) major)
	  (lambda (major minor) minor))])))

; mred:vertical-panel%.  See mred:horizontal-panel%, but reverse
; "horizontal" and "vertical."
(define mred:vertical-panel%
  (class-asi mred:container-panel%
    (public
      [get-min-size
	(make-get-size this
	  (lambda (curr-x spacing curr-info)
	    (max curr-x (+ (* 2 spacing)
			   (mred:child-info-x-min curr-info))))
	  (lambda (curr-y spacing curr-info)
	    (+ curr-y spacing (mred:child-info-y-min curr-info))))]
      [redraw
	(make-h-v-redraw this
	  mred:child-info-y-min
	  mred:child-info-y-stretch
...
; implement a panel which can hold multiple objects but only displays one
; at a time.  The size of the panel is the smallest size possible for
; displaying each of the panel's children.
(define mred:single-container-panel%
  (class-asi mred:container-panel%
    (public
	...
	(make-get-size this
	  (lambda (curr-x spacing info)
	    (max curr-x (mred:child-info-x-min info)))
	  (lambda (curr-y spacing info)
	    (max curr-y (mred:child-info-y-min info)))
	  0)]
      [redraw
	(lambda (width height)
	  (unless (null? active)
	    (let* ([active-info (send active get-info)]
		   [x-stretch (mred:child-info-x-stretch active-info)]
		   [x-min (mred:child-info-x-min active-info)]
		   [y-stretch (mred:child-info-y-stretch active-info)]
		   [y-min (mred:child-info-y-min active-info)]
		   [x-posn (if x-stretch
			       0
			       (/ (- width x-min) 2))]
		   [x-size (if x-stretch

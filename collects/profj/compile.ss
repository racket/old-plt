(module compile mzscheme
  ...
  (define (compile-java-internal port location type-recs file? level)
    ...
    (to-file file?)
    (printf "parse:")
    (let ((ast (time (parse port location level))))
      (printf "remember-main:")
      (time (remember-main ast))
      (printf "load-lang:")
      (time (load-lang type-recs))
      (printf "set-importer")
      (time (set-importer! type-recs find-implicit-import))
      (printf "build-inf:")
      (time (build-info ast level type-recs #f))
      (printf "check-defs:")
      (unless (null? (check-list))
        (time (check-defs (car (check-list)) level type-recs)))
      (printf "remove-from-packages")
      (time (remove-from-packages ast type-recs))
      (printf "translate:")
      (order-cus (time (translate-program ast type-recs)) type-recs)))

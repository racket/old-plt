...
The plotting library does something useful.. really...
...
After loading the correct module using \scheme{(require (lib "plot.ss" "plot"))} try \scheme{(plot (line (lambda (x) x)))}
...
Defining custom plots is simple : a renderer (that is passed to plot or mix) is just a function
       that acts on a \urlh{#view-snip} {\scheme{view-snip%}}. Both the 2d and 3d view snip have several drawing functions defined that
       ...
       in syntax.ss and renderer-helpers.ss that can be useful in creating your own renderers.


For example, if we wanted to create a function that creates renderers that 
...
\scheme{points} will draw points on a graph given a list. of vectors specifying their location. 


\scheme{line} will draw a line specified in either functional, ie. y=f(x), or  parametric mode, x,y = f(t) . If the function is parametric, the line-option \scheme{mode} must be set to \scheme{parametric}.
...
\scheme{field} will draw a vector field from a vector valued function.


Both \scheme{shade} and \scheme{contour} will render 3d functions on a 2d graph using colored shades and contours (respectively) to represent the value of the funciton at that position.
...
The \scheme{fit} form attempts to fit a a \scheme{fittable-function} to the data that is given.The \scheme{guess-list}should be an associative list of parameters and values. The more accurate  your initial guesses are, the more likely the fit is to succed. Any guesses not included in the list will be initialized to 1\scheme{x-data}, \scheme{y-data} (if provided) , \scheme{result-data} and \scheme{error-data} should be lists of equal size. If all the data points have equal weight
       the \scheme{error-data} can be replaced with \scheme{null}


\scheme{fit-lambda} is nessisary to distinguish between the formal arguments and fitting parameters. The first set of symbols us the formal arguments, while the second set is the parameters. For example, in a function such as f(x) = mx+b the x is the formal argument while the m and b are parameters
...
\item \scheme{2dplotview} refers to the name of the view object that the 
         renderer will be applied to by \scheme{plot}
...
\item The last set of parenthesized expressions sets keywords based arguments and their default
         values for the renderer-generator. To over-ride values the user needs to 

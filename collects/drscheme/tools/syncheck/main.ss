	...
	[make-root-area-container
	 (lambda (% parent)
      ...
      (public
        ...
        [syncheck:button-callback
         (lambda ()
           (if (thread-running? (ivar interactions-text user-thread))
               (letrec ([built-in?
			...
			;; color each exp
                         ...
                         (let* ([id (zodiac:varref-var var)]
                           ...
                           (send text syncheck:add-menu start finish 
                                 (lambda ()
                                   ...
                                   (let ([ht (cond
                                               [(drscheme:basis:beginner-language? (ivar interactions-text user-setting)) prims:beginning]
                                               [(drscheme:basis:intermediate-language? (ivar interactions-text user-setting)) prims:intermediate]
                                               [(drscheme:basis:advanced-language? (ivar interactions-text user-setting)) prims:advanced]
                                               [else #f])])
                                     (cond
               ...
               (mred:message-box
                "Check Syntax"
                "Evaluation has been killed, so Check Syntax cannot run")))])

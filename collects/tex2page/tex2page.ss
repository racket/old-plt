...
;tex2page
;Dorai Sitaram, April 1997
;for details, please see
;http://www.ccs.neu.edu/~dorai/tex2page/tex2page-doc.html

;

;some operating-system-dependent stuff.  I've
;tried to make it all automatic, but it may need
;personal TLC
...
;don't know why, but MetaPost is called mp in
;teTeX (all Unix TeXs?) and mpost in MikTeX (all
;Windows TeXs?)
...
;change following to #t if you don't want
;closing </p>

(define *use-closing-p-tag?* #t)

;MzScheme-version-specific

(define eval-if-mzscheme eval)

(eval-if-mzscheme
  (if (char=? (string-ref (version) 0) #\2) #f
      ;MzScheme version < 200 doesn't have define-syntax
      ;but does have define-macro.  Define a
      ;just-enough define-syntax so that this file will
      ;load
...
;
;change stuff below at your peril!
;
;
...
;(define *html-mdash* "--")
;(define *html-mdash* " &#151; ")
(define *html-mdash* " -- ") ;less confusing than w/o flanking spaces
(define *html-ndash* "-")
(define *html-rdquo* "''")
(define *html-rsquo* "'")

;but if your browser supports ligatures,
;you can get prettier versions of the 
;above
...
;(define *primitive-texframe* #f)

;(define *math-primitive-texframe* #f)

(define *scm-token-delims*
  (list #\( #\) #\[ #\] #\{ #\} #\' #\` #\" #\; #\, #\|))

(define *shell-cmd-prefix* "")
...
(define *bibitem-num* #f)

(define *comment-char* #f)
(define *css-port* #f)
...
(define *esc-char* #f)
(define *esc-char-std* #f)
(define *esc-char-verb* #\|)
(define *eval-file-count* #f)
(define *eval-file-stem* #f)
(define *external-label-tables* #f)
(define *external-programs* #f)

(define *figure-stack* #f)
(define *footnote-list* #f)
(define *footnote-sym* #f)
...
(define *img-file-count* #f)
(define *img-file-tally* #f)
(define *imgdef-file-count* #f)
...
(define alphabetic-week-day
  (lambda (i)
    (vector-ref *the-days* i)))

(define alphabetic-month
  (lambda (i)
    (vector-ref *the-months* (- i 1))))
  ...
  (lambda (n upcase?)
    ...
    (let ((roman-digits
      ...
      (let loop ((n n) (dd roman-digits) (s '()))
        (if (null? dd) (list->string (reverse! s))
            (let* ((d (car dd))
                   (val (car d))
...
;(define sort!
;  (lambda (s lt)
;    ;sorts list s in-place so that its elts
;    ;are in lt order from left to right
;    ;
;    (let ((n (- (length s) 1)))
;      (let loopi ((i 0))
;        (if (< i n)
;            (let loopj ((j (+ i 1)))
;              (if (<= j n)
;                  (let ((s_i (list-ref s i)) (s_j (list-ref s j)))
;                    (unless (lt s_i s_j)
;                      (set-car! (list-tail s i) s_j)
;                      (set-car! (list-tail s j) s_i))
;                    (loopj (+ j 1)))
;                  (loopi (+ i 1)))))))
;    s))
...
(define table-get
  (lambda (tbl k . d)
    (cond ((lassoc k (table.alist tbl) (table.equ tbl)) => cdr)
          ((pair? d) (car d))
          (else #f))))
...
(define write-label
  (lambda (e)
    (unless *label-port*
      (let ((f (string-append *aux-dir/* *jobname*
  ...
  (lambda (x)
    (unless *tex-aux-port*
      (let ((f (string-append *aux-dir/* *jobname*
        ...
        (ensure-file-deleted f)
        (set! *tex-aux-port* (open-output-file f))))
    (display x *tex-aux-port*)))
...
(define write-log
  (lambda (x)
    ...
    (when (> *write-log-index* *write-log-max*)
      (unless (or (and (char? x)
                       (not (char=? x #\newline))
                       (not (char=? x #\())
                       (not (char=? x #\{)))
                  (number? x))
    ...
    (unless (and (= *write-log-index* 0)
                 (char? x)
                 (char-whitespace? x))
      (when *log-port* (display x *log-port*))
      (display x)
      (flush-output)
      )
    (set! *write-log-index*
      (cond ((char? x)
            ...
            ((number? x)
             (+ *write-log-index*
                (if (< x 10) 1 2)))
            ((string? x)
             (+ *write-log-index* (string-length x)))
            (else
              (+ *write-log-index* 1))))))
...
(define get-char
  (lambda ()
    (let ((c (let ((b (bport.buffer *current-tex2page-input*)))
               (if (null? b)
  ...
  (lambda ()
    (if (find-chardef #\space)
        (when (= (the-count "\\TIIPobeyspacestrictly")  0)
          ;if inside obeyspace ignore at most _one_ space only
          ;(kludge)
          (let ((c (snoop-actual-char)))
            (cond ((eof-object? c) #t)
                  ((char-whitespace? c) (get-actual-char))
                  (else #t))))
        (let ((newline-active? (or (find-chardef #\newline)
                                   ;(eqv? *tex-format* 'texinfo)
                                   ))
                     ...
                     (cond ((eof-object? c) s)
                           ...
                           (else
                             (unless (or ;*in-verb?*
                               ...
                               ((if (eqv? *tex-format* 'texinfo)
                                    ignore-same-line-space
                                    ignorespaces)))
                             s)))))))
            (else (string #\\ c))))))
...
(define get-dimen
  (lambda ()
    (let* ((n (get-real))
           (c1 (get-actual-char))
           (c2 (get-actual-char)))
      (if (and (char-alphabetic? c1) (char-alphabetic? c2))
          (list n (string->symbol (string c1 c2)))
          (begin (eat-dimen) (list 0 'pt))))))
    ...
    (case (snoop-actual-char)
      ((#\`) (get-actual-char) (ignorespaces)
       (if (char=? (snoop-actual-char) #\\)
           ...
           (get-actual-char)))
      ((#\') (get-actual-char)
       (integer->char  (get-integer 8)))
      ((#\") (get-actual-char)
       (integer->char  (get-integer 16)))
...
(define eat-dimen
  (lambda ()
    ...
    (fluid-let ((*not-processing?* #t))
      (let loop ((first? #t))
        (let ((x (get-filename))) ;kludge
          (if (string=? x "") 'done
              (let ((x0 (string-ref x 0)))
                (cond ((or (char=? x0 #\.) (char=? x0 #\-))
                       (loop #f))
                      ((char-numeric? x0) (loop #f))
                      ;((and first? (string=? x "=")) (loop #t))
                      ;((and first? (char=? x0 #\=)) (loop #f))
                      ((char=? x0 *esc-char*) (toss-back-string x))
                      ((ormap (lambda (z) (string=? x z))
                         '("by" "ex" "in" "minus" "plus" "pt" "true" "truein"))
                       (loop #f))
        ...
        (let ((c (snoop-actual-char)))
          (cond ((eof-object? c) s)
                ((or (char-whitespace? c)
                     (memv c endenvironment-delims)) s)
                (else (get-actual-char)
                      (loop (cons c s))))))))))
...
(define do-global
  (lambda ()
    ...
    (let ((next (get-ctl-seq)))
      (cond ((ormap (lambda (z) (string=? next z))
               '("\\def" "\\edef"))
             (do-def #t))
            ((string=? next "\\let")
             (do-let #t))
...
(define do-subject
  (lambda ()
    ...
    (let ((fr (top-texframe))
      ...
      (write-aux `(!default-title ,(tex-string->html-string tit)))
      (tex-def "\\\\" '() "{\\break}" #f fr)
      (output-title tit)
      (egroup)
      ;(do-para)
  ...
  (lambda ()
    ...
    (let* ((tit (get-group))
           ...
           (html-tit
             (begin
               (tex-def "\\\\" '() "{}" #f fr)
               (tex-def "\\thanks" #t "\\TIIPgobblegroup" #f fr)
               (tex-string->html-string tit))))
      (egroup)
      (unless *title* (flag-missing-piece 'document-title))
      (write-aux `(!default-title ,html-tit))
      (toss-back-string tit)
...
(define do-maketitle
  (lambda ()
    ...
    (let ((fr (top-texframe)))
      (tex-def "\\\\" '() "\\break" #f fr)
      (tex-def "\\and" '() "\\break" #f fr)
      (tex-def "\\thanks" #t "\\symfootnote" #f fr))
...
(define do-css-block
  (lambda ()
    ...
    (let loop ()
      (let ((c (snoop-actual-char)))
        (cond ((eof-object? c) (terror 'do-css-block))
              ((char=? c *esc-char*)
               (let ((x (get-ctl-seq)))
                 (cond ((string=? x "\\endcssblock") 'done)
                       ((string=? x "\\end")
                        (cond ((get-grouped-environment-name-if-any) =>
                               (lambda (e)
                                 (unless (string=? e "cssblock")
                                   (write-char c *css-port*)
                                   (write-char #\{ *css-port*)
                                   (display e *css-port*)
                                   (write-char #\} *css-port*)
                                   (loop))))
                              (else (display x *css-port*)
                                (loop))))
                       (else (display x *css-port*)
                         (loop)))))
              (else (get-actual-char)
  ...
  (lambda (seclvl)
    (let ((unnumbered?
      ...
      (let* ((htmlnum (max 1 (min 6 (if *using-chapters?*
        ...
        (unless unnumbered?
          (tex-def-toks "\\TIIPrecentlabelname" lbl (top-texframe))
          (tex-def-toks "\\TIIPrecentlabelvalue" lbl-val (top-texframe))
          )
        (do-end-para)
        (emit "<a name=\"")
        (emit lbl)
        ...
        (let ((write-to-toc? ;kludge?
            ...
            ((-1)
             ...
             (if unnumbered? (emit-nbsp 1)
                 (begin
                   (when write-to-toc?
                     (emit "<a href=\"")
                     (emit (maybe-label-page *toc-page*))
                     (emit "%_toc_")
                     (emit lbl)
                     (emit "\">"))
                   (emit "Part ")
                   (emit lbl-val)
                   (when write-to-toc? (emit "</a>"))))
             (emit "</div><br>")
             (emit-newline))
            ((0)
             (emit-newline)
          ...
          (let ((tocdepth (get-gcount "\\tocdepth")))
            (when (and write-to-toc? (or (< tocdepth -1)
                                         (<= seclvl tocdepth)))
              (write-aux
                `(!toc-entry
                   ,(if (= seclvl -1) -1
...
(define do-documentclass
  (lambda ()
    ...
    (let ((x (string-trim-blanks (ungroup (get-group)))))
...
(define do-table
  (lambda ()
    (do-end-para)
    (bgroup)
    (set! *figure-stack* (cons 'table *figure-stack*))
    (set! *tabular-stack* (cons 'table *tabular-stack*))
    (get-bracketed-text-if-any)
    (let* ((counter (table-get *dotted-counters* "table"))
           (new-counter-value (+ 1 (counter.value counter))))
      (set!counter.value counter new-counter-value)
      (let* ((tbl-num (cond ((counter.within counter) =>
                                                      (lambda (sec-num)
                                                        (string-append
                                                          (section-counter-value sec-num) "."
                                                          (number->string new-counter-value))))
                            (else (number->string new-counter-value))))
             (tbl-tag (string-append "%_tbl_" tbl-num)))
        (tex-def-toks "\\TIIPrecentlabelname" tbl-tag (top-texframe))
        (tex-def-toks "\\TIIPrecentlabelvalue" tbl-num (top-texframe))
        (emit "<a name=\"")
        (emit tbl-tag)
        (emit "\"></a>")
        ;(emit "<hr>")
        (emit "<div class=table><table width=100%><tr><td align=")
...
(define do-figure
  (lambda ()
    ...
    (set! *figure-stack* (cons 'figure *figure-stack*))
    (set! *tabular-stack* (cons 'figure *tabular-stack*))
    (get-bracketed-text-if-any)
    (let ((fig-tag (string-append "%_fig_" (gen-temp-string))))
      (tex-def-toks "\\TIIPrecentlabelname" fig-tag (top-texframe))
      (emit "<a name=\"")
      (emit fig-tag)
      (emit "\"></a>")
      (emit-newline)
...
(define do-end-table
  (lambda ()
    ...
    (emit "</div>")
    ;(emit "<hr>")
    (set! *figure-stack* (cdr *figure-stack*))
...
(define do-end-figure
  (lambda ()
    (cond ((snoop-actual-char) =>
           (lambda (c)
             (if (char=? c #\*) (get-actual-char)))))
    (do-end-table)))

(define do-caption
  (lambda ()
    (let* ((counter (table-get *dotted-counters* "figure"))
           (new-counter-value (+ 1 (counter.value counter))))
      (set!counter.value counter new-counter-value)
      (let ((fig-num (cond ((counter.within counter)
                            =>
                            (lambda (sec-num)
                              (string-append (section-counter-value sec-num)
                                             ...
                                             (number->string new-counter-value))))
                           (else (number->string new-counter-value)))))
        (tex-def-toks "\\TIIPrecentlabelvalue" fig-num (top-texframe))
        (get-bracketed-text-if-any)
        (do-end-para)
        (emit "</td></tr><tr><td align=")
        (emit *display-justification*)
        (emit "><b>")
        (if (null? *figure-stack*)
            (terror 'do-caption "Mislaid \\caption")
            (case (car *figure-stack*)
              ((figure) (emit "Figure "))
              ((table) (emit "Table "))))
        (tex2page-string (get-toks "\\TIIPrecentlabelvalue"))
        (emit ":</b>")
        (emit-nbsp 2)
        (tex2page-string (get-group))
        (emit "</td></tr><tr><td>")))))
...
(define do-endminipage
  (lambda ()
    (set! *tabular-stack* (cdr *tabular-stack*))
    (let ((in-table? (and (not (null? *tabular-stack*))
                          (memv (car *tabular-stack*) '(figure table)))))
      (emit "</div>")
...
(define do-toc
  (lambda ()
    ...
    (when (eqv? *tex-format* 'latex)
      (tex2page-string
        (if *using-chapters?*
            "\\chapter*{Contents}" "\\section*{Contents}")))
    (emit "<a name=\"%_toc_start\">")
              ...
              (for-each
                (lambda (x)
                  (let* ((lvl (tocentry.level x))
                    ...
                    (emit seclabel)
                    (emit "\" href=\"")
                    (emit (maybe-label-page (tocentry.page x)))
                    (emit seclabel)
                    (emit "\">")
                    ;(when (= lvl -1) (emit "Part "))
                    (unless (string=? secnum "IGNORE")
                      (emit secnum)
                      ;(emit #\.)
                      (emit-nbsp 2))
                    (fluid-let ((*tabular-stack* (list 'header)))
...
(define number->footnote-symbol
  (lambda (n)
    (list-ref '("*" "\\dag" "\\ddag" "\\S" "\\P"
                 "\\Vert" "**" "\\dag\\dag" "\\ddag\\ddag")
      (modulo (- n 1) 9))))

(define do-plain-footnote
  (lambda ()
    (let ((fnmark (get-token)))
      (do-footnote-aux fnmark))))
  ...
  (lambda (fnmark)
    (let* ((fnlabel (gen-temp-string))
      ...
      (let ((old-html *html*))
        ...
        (tex-def-toks "\\TIIPrecentlabelname" fntag (top-texframe))
        (tex-def-toks "\\TIIPrecentlabelvalue" fnmark (top-texframe))
        ...
        (let loop ((i (- n 1)))
          (unless (< i 0)
            (let ((fv (list-ref *footnote-list* i)))
              (do-para)
              (emit "<a name=\"")
              (emit (footnotev.tag fv))
              (emit "\" href=\"#")
              (emit (footnotev.caller fv))
              (emit "\"><sup><small>")
    ...
    (add-postlude-to-top-frame
      (cond ((string=? sw "\\rm")
            ...
            ((ormap (lambda (z) (string=? sw z))
             ...
             (emit "<small>")
             (lambda () (emit "</small>")))
            ((string=? sw "\\fiverm") (emit "<font size=-2>")
             (lambda () (emit "</font>")))
            ((string=? sw "\\color")
             (emit "<font color=\"")
...
(define do-obeylines
  (lambda ()
    ...
    (tex-def-char #\newline '() "\\TIIPbr" (top-texframe))
    ;(add-postlude-to-top-frame do-para)
    ))

(define do-obeyspaces
  (lambda ()
    (tex-def-char #\space '() "\\TIIPnbsp" (top-texframe))))
...
(define do-block
  (lambda (z)
    (do-end-para)
    (emit "<div ")
    (cond ((string=? z "\\center") (emit "align=center"))
          ((string=? z "\\flushleft") (emit "align=left"))
          ((string=? z "\\flushright") (emit "align=right")))
    (emit ">")
...
(define do-function
  (lambda (fn)
    (cond ((string=? fn "\\emph") (emit "<em>"))
          ...
          (else (error 'do-function "Unknown function " fn)))
    (bgroup)
    (tex2page-string (get-group))
    (egroup)
    (cond ((string=? fn "\\emph") (emit "</em>"))
...
(define htmlize-label
  (lambda (lbl)
    (let* ((lbl (string-trim-blanks lbl))
           (n (string-length lbl)))
      (let loop ((i (- n 1)) (space? #f) (r '()))
        (if (< i 0) (list->string r)
          (let ((c (string-ref lbl i)))
            (cond ((char-whitespace? c)
                   (loop (- i 1) #t (if space? r (cons #\_ r))))
                  ((char=? c #\?)
                   (loop (- i 1) #f `(#\_ #\Q #\_ ,@r)))
                  ((char=? c #\:)
                   (loop (- i 1) #f `(#\_ #\C #\_ ,@r)))
...
(define get-label
  (lambda ()
    (htmlize-label (ungroup (get-group)))))
...
(define do-node
  (lambda ()
    (set! *recent-node-name* (get-label))))
  ...
  (lambda ()
    (let* ((f (let ((c (snoop-actual-char)))
      ...
      (when (file-exists? ext-label-file)
        (fluid-let ((*label-table* ext-label-table))
          (load ext-label-file))))))
...
(define do-tag
  (lambda ()
    (let ((tag-name (get-label)))
      (do-tag-aux tag-name
                  (get-group)))))

(define do-tag-aux
  (lambda (tag-name tag-val)
    (tex-def-toks "\\TIIPrecentlabelname" tag-name (top-texframe))
    (tex-def-toks "\\TIIPrecentlabelvalue" tag-val (top-texframe))
    (emit "<a name=\"")
    (emit tag-name)
    (emit "\"></a>")
    (do-label-aux tag-name)))

(define do-html-page-label
  (lambda ()
    (let ((label (get-label)))
...
(define do-ref-external
  (lambda ()
    (let ((ext-file (string-trim-blanks (ungroup (get-group)))))
    ...
    (let* ((label-table (if ext-file
           ...
           (label-text 
                       (cond (link-text (tex-string->html-string link-text))
                             (label-vec
                              (tex-string->html-string (vector-ref label-vec 2)))
                             (else label))))
      (emit "<a href=\"")
      (cond (label-vec
              (let ((pageno (vector-ref label-vec 0)))
                (cond (ext-file (emit ext-file)
                                (when (> pageno 0)
                                  (emit *html-page-suffix*)
                                  (emit pageno))
                                (emit ".html#"))
                      (else (emit (maybe-label-page pageno)))))
              (emit (vector-ref label-vec 1)))
             (else (emit *jobname*)
                   (emit ".hlog")))
      (emit "\">")
      (emit label-text)
...
(define do-htmlref-external
  (lambda ()
    (let* ((text (get-group))
           (extf (string-trim-blanks (ungroup (get-group))))
           (lbl (get-label)))
...
(define show-unresolved-xrefs
  (lambda ()
    (unless (null? *unresolved-xrefs*)
      ...
      (for-each (lambda (x) (write-log #\,) (write-log #\space)
                  (write-log x))
  ...
  (lambda ()
    (unless (null? *missing-pieces*)
      ...
      (when (memv 'toc *missing-pieces*)
        (write-log "Table of contents not generated")
        (write-log #\newline))
      (when (memv 'index *missing-pieces*)
        (write-log "Index not generated")
        (write-log #\newline))
      (when (memv 'bibliography *missing-pieces*)
        (write-log "Bibliography not generated")
        (write-log #\newline)))))
...
(define do-thebibliography
  (lambda ()
    ...
    (when (eqv? *tex-format* 'latex)
      (tex2page-string
        (if *using-chapters?*
            "\\chapter*{Bibliography}" "\\section*{References}")))
    (set! *bibitem-num* 0)
...
(do-end-para)
      ...
      (unless label-value
        (set! label-value (number->string *bibitem-num*)))
      (tex-def-toks "\\TIIPrecentlabelvalue" label-value (top-texframe))
      ...
      (let ((key (string-append "cite{" (get-label) "}")))
        ...
        (tex-def-toks "\\TIIPrecentlabelname" key (top-texframe))
        (emit "<a name=\"")
        (emit key)
        (emit "\"></a>[")
  ...
  (lambda ()
    (let ((idx-entry (ungroup (get-group))))
      (unless (substring? "|)" idx-entry)
        ...
        ;ranging but who TF remembers these things?
        ;
        (set! *index-alist*
              (cons (cons *index-count* *html-page-count*) *index-alist*))
        (let ((tag (string-append "%_idx_" (number->string *index-count*))))
          (emit "<a name=\"") (emit tag) (emit "\"></a>")
          (unless *index-port*
        ...
        (cond ((get-csv) =>
               (lambda (s)
                 (let* ((n (string->number s))
                        (pageno (assv n *index-alist*)))
                   (cond (pageno
                           (if first-link? (set! first-link? #f)
                               (emit ", "))
                           (emit "<a href=\"")
                           (emit (maybe-label-page (cdr pageno)))
                           (emit "%_idx_")
                           (emit s)
                           (emit "\">")
                           (if item
...
(define do-index-item
  (lambda (z)
    (emit "<br>") (emit-newline)
    (cond ((string=? z "\\indexsubitem") (emit-nbsp 4))
          ((string=? z "\\indexsubsubitem") (emit-nbsp 8)))
    ;
                 ...
                 (cond ((or (string=? x "\\subitem")
                       ...
                       (else
                         (let ((xr (reverse! (string->list x))))
                           (loop (if (char-alphabetic? (car xr))
...
(define do-bigskip
  (lambda (type)
    (do-para)
    (when (ormap (lambda (z) (string=? type z))
            '("\\bigskip" "\\bigbreak"))
      (emit "<br>")
      (do-para)
      (emit "<br>"))
...
;(define output-modification-time
;  (lambda ()
;    (when (and *timestamp?* *last-modification-time*
;               (> *last-modification-time* 0))
;      (do-end-para)
;      (emit "<div class=smallprint>")
;      (emit-newline)
;      ;(emit "<a href=\"http://www.ccs.neu.edu/~dorai/")
;      ;(emit "tex2page/tex2page-doc.html\">")
;      (emit "Last modified: ")
    ...
    (let ((first-page? (= *html-page-count* 0))
      ...
      (unless (and first-page? last-page?)
        ...
        (unless first-page?
          ...
          (emit ".html\">"))
        (emit "first")
        (unless first-page? (emit "</a>"))
        (emit ", ")
        ;
        (unless first-page?
          (emit "<a href=\"")
          (emit *jobname*)
          (unless (= *html-page-count* 1)
            (emit *html-page-suffix*)
            (emit (- *html-page-count* 1)))
          (emit ".html\">"))
        (emit "previous")
        (unless first-page? (emit "</a>"))
        ;(emit ", ")
        ...
        (when (or *toc-page* *index-page*)
          ...
          (when *index-page*
            ...
            (unless index-page?
              (emit "<a href=\"")
              (emit (maybe-label-page *index-page*))
              (emit "%_index_start\">"))
            (emit "index")
            (unless index-page? (emit "</a>"))
            (emit "</span>")))
        (emit "]</div>")
        (do-para)))))
...
(define do-eject
  (lambda ()
    ;first, an unspeakable kludge put in to
    ;placate story.tex, which although horrid as an
    ;example file, happens to be viewed as canonical
    ;by everyone looking at TeX
...
(define output-html-preamble
  (lambda (top-level-page?)
    ...
    (tex2page-string (or *title* *jobname*)) (emit-newline)
    (emit "</title>") (emit-newline)
    (for-each link-stylesheet *stylesheets*)
    (emit "<meta name=robots content=\"")
    (unless (and top-level-page? (= *html-page-count* 0)) 
      (emit "no"))
    (emit "index,follow\">")
    (emit-newline)
    ...
    (emit "<body")
    (if (null? *stylesheets*)
        ;(emit " bgcolor=#e5e5e5 text=#000000")
        (emit " bgcolor=#ffffff text=#000000")
        )
    (emit ">")
    (emit-newline)))
...
(define do-end-page
  (lambda ()
    ...
    (do-end-para)
    ;(when (= *html-page-count* *last-page-number*) (output-colophon))
...
(define do-bye
  (lambda ()
    ...
    (if *scm-port* (close-output-port *scm-port*))
    (if *tex-aux-port* (close-output-port *tex-aux-port*))
    ...
    (let ((num-pages (+ *html-page-count* 1)))
      ...
      (write-log (string-append *aux-dir/* *jobname* ".html"))
      (when (> num-pages 1)
        (write-log #\,)
        (write-log #\space)
        (write-log "..."))
...
(define do-diacritic
  (lambda (diac)
    (let* ((x (ungroup (get-token)))
      ...
      (cond ((and (memv c '(#\a #\e #\i #\o #\u #\A #\E #\I #\O #\U))
                  (ormap (lambda (z) (string=? diac z))
                    '("acute" "uml" "circ" "grave")))
             (emit "&") (emit c)
             (emit diac) (emit ";"))
            ((and (memv c '(#\a #\n #\o #\A #\N #\O))
                  (string=? diac "tilde"))
             (emit "&") (emit c) (emit "tilde;"))
            ((and (memv c '(#\c #\C)) (string=? diac "cedil"))
             (emit "&") (emit c)
             (emit "cedil;"))
            ((char=? c #\space)
             (emit (cond ((string=? diac "circ") "^")
                         ((string=? diac "tilde") "~")
                         (else "<u>&nbsp;</u>"))))
            (else (emit "<u>")
              (emit c)
...
(define do-imgless-display-math
  (lambda ()
    (if *math-mode?* (egroup)
...
(define do-imgless-in-text-math
  (lambda ()
    (if *math-mode?* (egroup)
        (let ((old-in-math? *math-mode?*)
              (old-in-display-math? *in-display-math?*)
...
(define call-with-image-port
  (lambda (p)
    (set! *img-file-count* (+ *img-file-count* 1))
    (let* ((img-file-stem (string-append *subjobname* *img-file-suffix*
    ...
    (source-img-file      
      (call-with-image-port
        (lambda (o)
          ...
          (dump-till-char #\$ o)
          (let ((c (get-actual-char)))
            (cond ((eof-object? c) (terror 'do-img-display-math))
                  ((char=? c #\$) #t)
                  (else (terror 'do-img-display-math))))
          (display "$$" o))))
    (emit "</div>")
    (do-para)))
 
(define do-img-in-text-math
  (lambda ()
      (source-img-file
      (call-with-image-port
        (lambda (o)
...
(define do-math
  (lambda ()
    (let ((display? #f))
      ...
      ((if *use-img-for-math?*
           (if display?
               (if *use-img-for-display-math?*
                   do-img-display-math
                   do-imgless-display-math)
               (if *use-img-for-in-text-math?*
                   do-img-in-text-math
                   do-imgless-in-text-math))
           (if display?
               do-imgless-display-math
               do-imgless-in-text-math))))))
...
(define dump-till-char
  (lambda (d o)
    (let loop ((nesting 0) (escape? #f))
      (let ((c (get-actual-char)))
        (cond ((eof-object? c) (terror 'dump-till-char))
...
(define do-imgpreamble
  (lambda ()
    ;(if *imgpreamble*
    ;    (terror 'do-imgpreamble "Calling \\imgpreamble more than once"))
    (set! *imgpreamble*
      (fluid-let ((*not-processing?* #t))
        (let loop ((r *imgpreamble*))
          (let ((c (snoop-actual-char)))
            (cond ((eof-object? c)
                   (terror 'do-imgpreamble "Missing \\endimgpreamble"))
                  ((char=? c *esc-char*)
                   (let ((x (get-ctl-seq)))
                     (cond ((ormap (lambda (z) (string=? x z)) '("\\endimgpreamble"
                                         "\\endgifpreamble"
                                         "\\endmathpreamble"))
                            r)
...
(define do-unknown-if
  (lambda ()
    (set! *tex-if-stack* (cons '? *tex-if-stack*))))
    ...
    (let* ((one (get-raw-token/is))
      ...
      ((if (string=? one two) do-iftrue
           (begin
             (when (ctl-seq? one)
               (set! one
                 (cond ((find-def one) => caddr)
                       ((find-math-def one) => (lambda (x) x))
                       (else 'undefined))))
             (when (ctl-seq? two)
               (set! two
                 (cond ((find-def two) => caddr)
...
(define do-html-img-format
  (lambda ()
    (set! *img-format*
      (string->symbol (ungroup (get-group))))))

(define do-html-img-magnification
  (lambda ()
    (set! *img-magnification*
      (string->number (ungroup (get-group))))))

(define call-tex
  (lambda (f)
    ;run tex on f and return #t if a proper .dvi
    ;file was produced
    (let ((dvifile (string-append f ".dvi"))
          (call-to-tex (string-append
                     ...
                     (let ((x (read-line i)))
                       (cond ((eof-object? x)
                             ...
                             ((substring? "! I can't find file" x)
                              ...
                              #f)
                             ((substring? "MFpic: Don't forget to process " x)
                              ;contains mfpic's --
                              ;so call metapost and rerun tex
                              (system
                                (string-append *metapost* " "
                                  *jobname* *img-file-suffix*
                                  (number->string *img-file-count*) "-M"))
                              (close-input-port i)
                              (system call-to-tex)
                              #t)
                             (else (loop))))))))))))
...
(define *img-file-extn*
  (case *img-format*
    ...
    ((png) ".png")
    ((jpeg) ".jpeg")
    (else (set! *img-format* 'gif) ".gif")))

(define ps-to-img/gif
  (lambda (ps-file f)
    (system
      (string-append *ghostscript* *ghostscript-options*
                     " -sDEVICE=ppmraw -sOutputFile=" f ".ppm.1 " ps-file " quit.ps"))
    (system
  ...
  (lambda (f)
    ...
    (let ((f.img (string-append *aux-dir/* f *img-file-extn*)))
      (unless (file-exists? f.img)
        (write-log #\space)
        (write-log #\{)
        (write-log (string-append *aux-dir/* f ".tex"))
        (write-log #\space)
        (write-log "->")
        (write-log #\space)
        (cond ((call-tex f)
               (system
                 (string-append "dvips " f ".dvi -o " f ".ps"))
...
(define do-opengraphsfile
  (lambda ()
    (set! *current-mfpic-file-stem*
      (string-trim-blanks (ungroup (get-group))))
    (set! *current-mfpic-file-num* 0)))
...
;;;         (let ((c (snoop-actual-char)))
;;;           (cond ((eof-object? c)
;;;                  (terror 'do-mfpic "Eof inside \\mfpic"))
;;;                 ((char=? c *esc-char*)
;;;                  (unless (string=? (get-ctl-seq) "\\endmfpic")
;;;                    (loop)))
;;;                 (else (get-actual-char)
;;;                   (loop))))))
;;;     ;
;;;     (set! *current-mfpic-file-num*
;;;       (+ *current-mfpic-file-num* 1))
;;;     (let ((img-file-stem (string-append
;;;                            *current-mfpic-file-stem* *mfpic-file-suffix*
;;;                            (number->string *current-mfpic-file-num*))))
;;;       (unless (file-exists? (string-append img-file-stem *img-file-extn*))
;;;         (ps-to-img (string-append *current-mfpic-file-stem*
;;;                      "." (number->string *current-mfpic-file-num*))
;;;           img-file-stem))
;;;       (source-img-file img-file-stem))))

;above definition would've been nice as it reuses the EPS
;files generated for print, but it doesn't catch
;\tlabels.  The following runs TeX on individual
;subfiles, which means you have to make sure you put
;everything used by the subfile into the image
;preamble.  Usually not a problem, since mfpic contents
;typically don't use TeX definitions in the rest
;of the doc
     ...
     (call-with-image-port
      (lambda (o)
        (fluid-let ((*not-processing?* #t))
          (let ((img-file-stem (string-append
                                *current-mfpic-file-stem* *mfpic-file-suffix*
                                (number->string *current-mfpic-file-stem*))))
            (display "\\input mfpic" o) (newline o)
            (display "\\usemetapost" o) (newline o)
            (display "\\opengraphsfile{" o)
            (display img-file-stem o)
            (display "-M}" o) (newline o)
            (display "\\mfpic" o)
            (dump-till-end-env "mfpic" o)
            ;(dump-till-ctl-seq "\\endmfpic" o)
            '(let loop ()
               (let ((c (snoop-actual-char)))
                 (cond ((eof-object? c)
                        (terror 'do-mfpic "Eof inside \\mfpic"))
                       ((char=? c *esc-char*)
                        (let ((x (get-ctl-seq)))
                          (unless (string=? x "\\endmfpic")
                            (display x o)
                            (loop))))
                       (else (display (get-actual-char) o)
                             (loop)))))
            (display "\\endmfpic" o) (newline o)
            (display "\\closegraphsfile" o) (newline o)
            )))))
    ;(system (string-append "tex " img-file-stem))
    ;(system (string-append *metapost* " " img-file-stem "-M"))
    ))
          ...
          (source-img-file
            (call-with-image-port
              (lambda (o)
                ...
                (display f o)
                (display #\} o))))
          (non-fatal-error "PostScript file " f " missing")))))

;      (unless (file-exists? (string-append img-file-stem *img-file-extn*))
;        (ps-to-img eps-file img-file-stem)
;        (move-aux-files-to-aux-dir img-file-stem)
;        (source-img-file img-file-stem))
      ...
      (call-with-image-port
        (lambda (o)
          (fluid-let ((*not-processing?* #t))
            ...
            (display "\\end{picture}" o) (newline o)
            ))))))
...
(define do-eqalign
  (lambda ()
    (ignorespaces)
    (let ((c (get-actual-char)))
      (if (eof-object? c) (terror 'do-eqalign "Missing {"))
      (unless (char=? c #\{) (terror 'do-eqalign "Missing {"))
      ;
      (bgroup)
      (set! *tabular-stack* (cons 'eqalign *tabular-stack*))
      (add-postlude-to-top-frame
        (lambda ()
          (emit "</td></tr></table>")
...
(define find-some-file
  (lambda (path . files)
    ;look through each directory in path until one
    ;of files is found.  Always look in current
    ;directory first, whether or not it's specified
    ;in path
...
;  (lambda (f)
;    ;convert filename f to an ``actual'' filename, just like tex
;    ;does
;    (if (file-extension f) f
;      (let ((f2 (string-append f ".tex")))
;        (cond ((file-exists? f2) f2)
;              ((file-exists? f) f)
;              (else f))))))
...
(define actual-tex-filename
  (lambda (f)
    ...
    ;search TIIPINPUTS.  (Can't use TEXINPUTS, because
    ;modern TEXINPUTS spec is too complicated for
    ;R5RS)
    (or (find-some-file *tex2page-inputs*
          (string-append f ".tex") f)
        f)))

(define ignore-tex-specific-text
  (lambda (z)
    (let* ((env (substring z 1 (string-length z)))
                       ...
                       ((string=? x "\\input")
                        (let ((f (get-filename)))
                          (call-with-input-file/buffered f
                            ;can't eta-reduce because of CL
                            (lambda ()
                                (do-html-head-only)))
                          (loop '())))
...
(define resolve-defs
  (lambda (x)
    (cond ((find-def x) =>
           (lambda (y)
             (let ((args (cadr y)) (expn (caddr y)) (opt-arg (cadddr y)))
               (case args
                 ((:prim) expn) ;x or expn?
                 ((:thunk) x) 
                 (else
                  (cond ((and (inside-false-world?)
                              ...
                              (> (length args) 0)) x)
                        (else
                         (expand-tex-macro args expn opt-arg))))))))
          (else x))))
...
(define tex-def-char
  (lambda (char args expansion frame)
    (unless (and (eqv? frame *global-texframe*)
                 (or (char=? char #\space) (char=? char #\newline)))
      ;too dangerous for t2p to define #\space and #\newline
      ;in *global-texframe* although this is commonly done in tex
      (cond ((assoc char (texframe.chardefinitions frame)) =>
             (lambda (c)
               (set-car! (cdr c) args)
               (set-car! (cddr c) expansion)))
            (else
              (set!texframe.chardefinitions frame
                (cons (list char args expansion)
                  (texframe.chardefinitions frame))))))))

(define tex-def
  (lambda (name args expansion opt-arg frame)
    (cond ((lassoc name (texframe.definitions frame) string=?) =>
           (lambda (c)
             (set-car! (cdr c) args)
             (set-car! (cddr c) expansion)
             (set-car! (cdddr c) opt-arg)))
          (else
            (set!texframe.definitions frame
              (cons (list name args expansion opt-arg)
                (texframe.definitions frame)))))))

(define tex-def-prim
  (lambda (cs th)
    (set!texframe.definitions *primitive-texframe*
      (cons (list cs ':thunk th #f)
        (texframe.definitions *primitive-texframe*)))))

(define tex-let-prim
  (lambda (cs-new cs-old)
    (cond ((lassoc cs-old (texframe.definitions *primitive-texframe*)
             string=?) =>
           (lambda (cs-old-entry)
             (let ((cs-old-args (cadr cs-old-entry))
                   (cs-old-expn (caddr cs-old-entry))
                   (cs-old-optarg (cadddr cs-old-entry)))
               (cond ((lassoc cs-new
                        (texframe.definitions *primitive-texframe*) string=?) =>
                      (lambda (cs-new-entry)
                        (set-car! (cdr cs-new-entry) cs-old-args)
                        (set-car! (cddr cs-new-entry) cs-old-expn)
                        (set-car! (cdddr cs-new-entry) cs-old-optarg)))
                     (else
                       (set!texframe.definitions *primitive-texframe*
                         (cons (list cs-new cs-old-args
                                 cs-old-expn cs-old-optarg)
                           (texframe.definitions *primitive-texframe*))))))))
          (else (terror 'tex-let-prim cs-old " not defined")))))
...
(define tex-def-thunk
  (lambda (name thunk frame)
    (cond ((lassoc name (texframe.definitions frame) string=?) =>
           (lambda (c)
             (set-car! (cdr c) ':thunk)
             (set-car! (cddr c) thunk)
             (set-car! (cdddr c) #f)))
          (else
            (set!texframe.definitions frame
              (cons (list name ':thunk thunk #f)
                (texframe.definitions frame)))))))

(define tex-def-count
  (lambda (name num frame)
    (cond ((lassoc name (texframe.counts frame) string=?) =>
           (lambda (c)
             (set-car! (cdr c) num)))
          (else
            (set!texframe.counts frame
              (cons (list name num)
                (texframe.counts frame)))))))

(define tex-def-count-global
  (lambda (name num)
    (tex-def-count name num *global-texframe*)))

(define tex-def-toks
  (lambda (name tokens frame)
    (cond ((lassoc name (texframe.toks frame) string=?) =>
           (lambda (c)
             (set-car! (cdr c) tokens)))
          (else
            (set!texframe.toks frame
              (cons (list name tokens)
                (texframe.toks frame)))))))

(define tex-def-dimen
  (lambda (name frame)
    (cond ((lassoc name (texframe.dimens frame) string=?) #t)
          (else
            (set!texframe.dimens frame
...
(define initialize-global-texframe
  (lambda ()
    ...
    ;
    (tex-def-count-global "\\language" 256)
    ;
    ;the lowest section depth, the part, is -1
    ;so -2 is the closest-to-0 number that
    ;is a meaningless depth
    ;
    ;(tex-def-count-global "\\secnumdepth" -2)
    (tex-def-count-global "\\tocdepth" -2)
    (tex-def-count-global "\\footnotenumber" 0)
    (tex-def-count-global "\\TIIPtabularborder" 1)
    (tex-def-count-global "\\TIIPnestedtabularborder" 0)
    (tex-def-count-global "\\TIIPobeyspacestrictly" 0)
    ;
    (tex-def-toks "\\TIIPrecentlabelname" "" *global-texframe*)
    (tex-def-toks "\\TIIPrecentlabelvalue" "" *global-texframe*)
    ;
...
(define find-chardef
  (lambda (char)
    (or (ormap (lambda (fr)
                 (assoc char (texframe.chardefinitions fr)))
          *tex-env*)
        (assoc char (texframe.chardefinitions *global-texframe*))
        (assoc char (texframe.chardefinitions *primitive-texframe*)))))
...
(define find-def
  (lambda (ctlseq)
    (or (ormap (lambda (fr)
          ...
          *tex-env*)
        (lassoc ctlseq (texframe.definitions *global-texframe*)
          string=?)
        (lassoc ctlseq (texframe.definitions *primitive-texframe*)
          string=?))))
...
(define find-count
  (lambda (ctlseq)
    (or (ormap (lambda (fr)
                 (lassoc ctlseq (texframe.counts fr) string=?))
          *tex-env*)
...
(define find-toks
  (lambda (ctlseq)
    (or (ormap (lambda (fr)
                 (lassoc ctlseq (texframe.toks fr) string=?))
          *tex-env*)
...
(define find-dimen
  (lambda (ctlseq)
    (or (ormap (lambda (fr)
                 (lassoc ctlseq (texframe.dimens fr) string=?))
          *tex-env*)
...
(define get-toks
  (lambda (ctlseq)
    (cond ((find-toks ctlseq) => cadr)
          (else (terror 'get-toks)))))
...
(define set-gcount!
  (lambda (ctlseq v)
    (tex-def-count ctlseq v *global-texframe*)))

(define do-number
  (lambda ()
    (emit (the-count (get-ctl-seq)))))

(define do-the
  (lambda ()
    ;almost like do-number
    (let ((ctlseq (get-ctl-seq)))
      (cond ((find-count ctlseq) =>
             (lambda (x) (emit (cadr x))))
            ((find-toks ctlseq) =>
             (lambda (x)
               (tex2page-string (cadr x))))
  ...
  (lambda (global?)
    (unless (inside-false-world?)
      (let* ((lhs (get-raw-token/is))
             (arg-pat (get-def-arguments lhs))
             (rhs (ungroup (get-group)))
             (fr (if global? *global-texframe* (top-texframe))))
        (if (ctl-seq? lhs)
            (tex-def lhs arg-pat rhs #f fr)
            (tex-def-char (string-ref lhs 0) arg-pat rhs fr))))))

(define do-newcount
  (lambda (global?)
    (tex-def-count (get-ctl-seq) 0
      (if global? *global-texframe* (top-texframe)))))
...
(define do-multiply
  (lambda (global?)
    (let* ((ctlseq (get-ctl-seq))
           (curr-val (cadr (find-count ctlseq))))
      (get-by)
      (tex-def-count ctlseq
        (* curr-val (get-number))
        (if global? *global-texframe*
...
(define do-catcode
  (lambda ()
    (let* ((c (get-tex-char-spec))
      ...
      (case val
        ((0) (set! *esc-char* c))
        ((13) (tex-def-char c '() "" (top-texframe)))
        (else (let ((y (or (ormap (lambda (fr)
                                    (assoc c
                                      (texframe.chardefinitions fr)))
                             *tex-env*)
                           (assoc c (texframe.chardefinitions
                                      *global-texframe*)))))
                (if y (set-car! (cddr y) #f))))))))
...
(define do-defcsactive
  (lambda ()
    ...
    (let* ((c (get-ctl-seq))
           (rhs (ungroup (get-group)))
           (fr (top-texframe)))
      (tex-def-char (string-ref c 1) '() rhs fr))))

(define do-undefcsactive
  (lambda ()
    (ignorespaces)
    (let* ((c (string-ref (get-ctl-seq) 1))
           (fr (top-texframe))
           (y (assoc c (texframe.chardefinitions fr))))
      (if y (set-car! (cddr y) #f)
          (tex-def-char c '() #f fr)))))
...
(define do-newcommand
  (lambda (renew?)
    ...
    (let* ((lhs (string-trim-blanks (ungroup (get-token))))
      ...
      (tex-def lhs (latex-arg-num->plain-arg-pat argc)
        rhs opt-arg fr)
      (unless ok-to-def?
...
(define do-advancetally
  (lambda (global?)
    (let* ((ctlseq (get-ctl-seq))
      ...
      (tex-def ctlseq '()
        (number->string
          (+ (string->number (resolve-defs ctlseq))
             increment))
        #f
        (if global?
            *global-texframe*
...
(define do-newenvironment
  (lambda (renew?)
    ...
    (let* ((envname (string-trim-blanks (ungroup (get-token))))
      ...
      (tex-def bs-envname
        (latex-arg-num->plain-arg-pat argc) beginning optarg fr)
      (tex-def (string-append "\\end" envname) '() ending #f fr)
      (unless ok-to-def?
  ...
  (lambda ()
    (let* ((env (ungroup (get-group)))
      ...
      (tex-def (string-append "\\" env) '()
        (string-append "\\par\\begingroup\\TIIPtheorem{" counter-name "}{" caption "}")
        #f *global-texframe*)
      (tex-def (string-append "\\end" env) '()
        "\\endgroup\\par" #f *global-texframe*))))
        ...
        (let* ((thm-num (let ((sec-num (counter.within counter)))
                          (if sec-num
                              (string-append (section-counter-value sec-num) "."
                                (number->string new-counter-value))
                              (number->string new-counter-value))))
               (lbl (string-append "%_thm_" thm-num)))
          (tex-def-toks "\\TIIPrecentlabelname" lbl (top-texframe))
          (tex-def-toks "\\TIIPrecentlabelvalue" thm-num (top-texframe))
          (emit "<a name=\"")
          (emit lbl)
          (emit "\"></a>")
          (emit-newline)
...
(define do-end
  (lambda ()
    (cond ((get-grouped-environment-name-if-any) =>
          ...
          (else
            (toss-back-char *invisible-space*)
            (toss-back-string "\\bye")))))

(define latex-arg-num->plain-arg-pat
  (lambda (n)
    (let loop ((n n) (s '()))
      (if (<= n 0) s
          (loop (- n 1)
    ...
    (unless (inside-false-world?)
      ...
      (let* ((lhs (get-ctl-seq))
             (rhs (begin (get-equal-sign)
                         (get-raw-token/is))))
        (unless (and (ormap (lambda (z) (string=? lhs z))
                            '("\\texonly" "\\endtexonly"))
                     (string=? rhs "\\relax"))
          ;
          ;this is to prevent the loading of tex2page.tex
          ;from messing things up.  tex2page.tex contains
          ; \let\texonly\relax and \let\endtexonly\relax
          ;
          (let ((fr (if global? *global-texframe* (top-texframe))))
            (if (ctl-seq? rhs)
                (let ((s (find-def rhs)))
                  (if s (tex-def lhs (cadr s) (caddr s) (cadddr s) fr)
                      (tex-def lhs ':prim rhs #f fr)))
                (tex-def lhs '() rhs #f fr))))))))
...
(define make-reusable-img
  (lambda (global?)
    ...
    (ignorespaces)
    (let ((lhs (get-ctl-seq))
          (imgdef-file-stem (string-append *subjobname*
      ...
      (tex-def lhs '()
        (string-append "\\TIIPreuseimage{" imgdef-file-stem "}") #f fr))))
    ...
    (let* ((img-file (string-append img-file-stem *img-file-extn*))
      ...
      (cond ((file-exists? f)
            ...
            (else
              ...
              (write-log "missing") 
              (write-log #\))
              (non-fatal-error "Image not generated")
              #f)))))
...
(define expand-tex-macro
  (lambda (arg-pat rhs opt-arg)
    ;(unless (or *in-verb?* *math-mode?*)
    ;     (ignorespaces))
    (let* ((arg-pat-n (length arg-pat))
           (k 0)
           (r (if (not opt-arg) '()
                  (begin (set! k 2)
           ...
           (args
             (reverse!
               (let loop ((k k) (r r))
                 (if (>= k arg-pat-n) r
                     (let ((c (list-ref arg-pat k)))
                       (cond ((char=? c #\#)
                              (loop (+ k 2)
                                (cons
                                  (if (or (= (+ k 2) arg-pat-n)
                                          (char=? (list-ref arg-pat (+ k 2))
                                            #\#))
                                      (ungroup (get-token))
                                      (get-till-char
                                        (list-ref arg-pat (+ k 2))))
                                  r)))
                             ((char-whitespace? c)
                              (ignorespaces)
                              (loop (+ k 1) r))
                             (else
                               (let ((c2 (get-actual-char)))
                                 (if (eof-object? c2)
                                     (terror 'expand-tex-macro
                                       "Eof before macro got enough args")
                                     (if (char=? c c2) (loop (+ k 1) r)
                                         (terror
                                           'expand-tex-macro
...
(define set-verbatim-escape-character
  (lambda ()
    (ignorespaces)
    (let* ((c1 (get-actual-char))
           (c2 (get-actual-char)))
      (unless (char=? c1 *esc-char*)
        (terror 'set-verbatim-escape-character "Arg must be \\<char>"))
...
(define do-braced-verb
  (lambda ()
    ...
    (let ((in-text? (not (munched-a-newline?))))
      (if in-text? (emit "<code class=verbatim>")
      ...
      (fluid-let ((*esc-char* *esc-char-verb*)
        ...
        (let loop ((nesting 0))
          (let ((c (get-actual-char)))
            (cond ((eof-object? c)
                   (terror 'do-braced-verb "Eof inside verbatim"))
                  ((char=? c *esc-char*)
                   (toss-back-char c)
                  ...
                  (else (emit-html-char c)
                    (loop nesting))))))
      (egroup)
      (if in-text? (emit "</code>")
          (begin (emit "</pre>") (do-para))))))
 
(define do-latex-verb
  (lambda ()
    (let* ((d (get-actual-char))
           (in-text? (not (munched-a-newline?))))
      (if in-text? (emit "<code class=verbatim>")
          (begin (do-end-para) (emit "<pre class=verbatim>")))
      (let loop ()
        (let ((c (get-actual-char)))
          (cond ((eof-object? c)
                 (terror 'do-latex-verb "Eof inside verbatim"))
                ((char=? c d) 'done)
                (else (emit-html-char c)
                  (loop)))))
      (if in-text? (emit "</code>")
    ...
    (let ((d (get-actual-char)))
      (case d
        ((#\{) (let loop ((nesting 0))
                 (let ((c (get-actual-char)))
                   (cond ((eof-object? c)
                          (terror 'verb-write-to-file "Eof inside verbatim"))
                         ((char=? c *esc-char-verb*)
                          (write-char c *verb-port*)
                          (write-char (get-actual-char) *verb-port*)
                          (loop nesting))
                         ((char=? c #\{)
                          (write-char c *verb-port*)
                          (loop (+ nesting 1)))
                ...
                ((char=? c #\\)
                 (let ((end? (get-ctl-seq)))
                   (if (string=? end? "\\end")
                       (cond ((get-grouped-environment-name-if-any) =>
                              (lambda (e)
                                (unless (string=? e "verbatim")
                                  (emit-html-string end?)
                                  ;(for-each emit-html-char (string->list end?))
                                  (emit-html-char #\{)
                                  (emit-html-string e)
                                  ;(for-each emit-html-char (string->list e))
                                  (emit-html-char #\})
                                  (loop))))
                             (else (for-each emit-html-char end?)
                               (loop)))
                       (begin
                         (emit-html-string end?)
                         ;(for-each emit-html-char (string->list end?))
...
(define do-scm-set-special-symbol
  (lambda ()
    (let* ((sym (string-trim-blanks (ungroup (get-group))))
           (xln (get-group)))
      (table-put! *scm-special-symbols* sym xln))))

(define do-scm-unset-special-symbol
  (lambda ()
...
;    (let* ((sym (string-trim-blanks (ungroup (get-group)))))
;      (table-put! *scm-special-symbols* sym #f))

(define do-scm-set-constants
  (lambda ()
  ...
  (lambda ()
    ...
    (let ((c (snoop-actual-char)))
      (cond ((eof-object? c)
            ...
            (else (emit "<span class=selfeval>")
                  (scm-emit-html-char #\#)
                  (scm-display-token (scm-get-token))
                  (emit "</span>"))))))
    ...
    (let ((in-text? (not (munched-a-newline?))))
      ...
      (fluid-let ((*esc-char* *esc-char-verb*))
        (let loop ((nesting 0))
          (let ((c (snoop-actual-char)))
            (cond ((eof-object? c)
                   (terror 'do-scm-with-brace "Eof inside verbatim"))
                  ((char=? c *esc-char*)
                   (let ((x (fluid-let ((*not-processing?* #t))
                              (get-ctl-seq))))
                     (cond ((ormap (lambda (z) (string=? x z))
...
(define do-scm-with-delim
  (lambda (result?)
    (let ((d (get-actual-char)))
      (let ((in-text? (not (munched-a-newline?))))
        (cond  (in-text? (emit "<code class=scheme")
                         (when result? (emit "response"))
                         (emit ">"))
               (else (do-end-para) (emit "<pre class=scheme>")))
        (fluid-let ((*scm-token-delims* (cons d *scm-token-delims*)))
          (let loop ()
            (let ((c (snoop-actual-char)))
              (cond ((eof-object? c)
...
(define do-scm-set-filename
  (lambda ()
    (let ((f (get-filename)))
      (if *scm-port* (close-output-port *scm-port*))
      (ensure-file-deleted f)
      (set! *scm-port* (open-output-file f)))))

(define scm-ensure-output-port
  (lambda ()
    (unless *scm-port*
      (let ((output-file (string-append *jobname* ".scm")))
        (ensure-file-deleted output-file)
        (set! *scm-port*
          (open-output-file output-file))))))

(define do-scm-dribble
  (lambda ()
    (scm-ensure-output-port)
    (fluid-let ((*scm-dribbling?* #t))
      (do-scm #f))
    (newline *scm-port*)))

(define do-scm-write-to-file
  (lambda ()
    (scm-ensure-output-port)
    (ignorespaces)
    (let ((d (get-actual-char)))
      (case d
        ((#\{) (let loop ((nesting 0))
                 (let ((c (get-actual-char)))
                   (cond ((eof-object? c)
                          (terror 'do-scm-write-to-file
                            "Eof inside verbatim"))
                         ((char=? c *esc-char-verb*)
                          (write-char c *scm-port*)
                          (write-char (get-actual-char) *scm-port*)
                          (loop nesting))
                         ((char=? c #\{)
                          (write-char c *scm-port*)
                          (loop (+ nesting 1)))
        ...
        (let loop ()
          (let ((c (snoop-actual-char)))
            (cond ((eof-object? c)
                  ...
                  ((char=? c #\newline) (get-actual-char)
                   (scm-emit-html-char c)
                   (cond ((not *slatex-like-comments?*) #f)
...
(define do-cr
  (lambda (z)
    (ignorespaces)
    (let ((top-tabular (if (pair? *tabular-stack*)
...
(define do-tex-logo
  (lambda ()
    (emit "T<small>E</small>X")))

(define do-latex-logo
  (lambda ()
    (emit "L<small>A</small>")
    (do-tex-logo)))

(define do-latex2e-logo
  (lambda ()
    (do-latex-logo)
    (emit "2<small>E</small>")))

(define do-romannumeral
  (lambda (upcase?)
    (emit (number->roman (get-integer 10) upcase?))))

;(define do-setcounter
;  (lambda ()
;    (let* ((counter-name (string-trim-blanks (ungroup (get-group))))
;           (new-value (string->number (string-trim-blanks
;                                        (ungroup (get-group))))))
;      (cond ((string=? counter-name "secnumdepth")
;             (set-gcount! "\\secnumdepth" new-value))
;            ((string=? counter-name "tocdepth")
;             (set-gcount! "\\tocdepth" new-value))
;            ((table-get *dotted-counters* counter-name) =>
;             (lambda (counter)
;               (set!counter.value counter new-value)))
;            (else ;(terror 'do-setcounter)
;              #f)))))
  ...
  (lambda (add?)
    (let* ((counter-name (string-trim-blanks (ungroup (get-group))))
           (new-value (string->number (string-trim-blanks
                                        (ungroup (get-group))))))
      (cond ((string=? counter-name "secnumdepth")
             (set-gcount! "\\secnumdepth"
               (if add?
                   (+ new-value (get-gcount "\\secnumdepth"))
                   new-value)))
            ((string=? counter-name "tocdepth")
             (set-gcount! "\\tocdepth"
               (if add?
                   (+ new-value (get-gcount "\\tocdepth"))
                   new-value)))
            ((table-get *dotted-counters* counter-name) =>
             (lambda (counter)
    ...
    (cond ((find-def z) =>
           (lambda (y)
             (let ((args (cadr y)) (expn (caddr y)) (opt-arg (cadddr y)))
               (case args
                 ((:thunk) (expn))
                 ((:prim) (toss-back-string expn))
                 (else (expand-tex-macro args expn opt-arg))))))
          ((section-ctl-seq? z) => (lambda (n) (do-heading n)))
          (*math-mode?* (do-math-ctl-seq z))
          (else
            (trace-if *tracingcommands?* "Ignoring " z)))))
...
(define do-tex-char
  (lambda (c)
    (cond ((and *comment-char* (char=? c *comment-char*))
          ...
          ((char=? c #\>) (emit "&gt;"))
          ((char=? c #\") (emit "&quot;"))
          ((char=? c #\&) (cond ((pair? *tabular-stack*)
                                 (do-end-para)
                                 (case (car *tabular-stack*)
                                   ((eqalign pmatrix) (emit "</td><td>"))
                                   ((tabular) (do-tabular-colsep))
                                   ((ruled-table) (do-ruledtable-colsep))))
                                (else (emit "&amp;"))))
          ((char=? c #\|) (if (and (pair? *tabular-stack*)
...
(define do-tex-ctl-seq-completely
  (lambda (x)
    (let ((z (resolve-defs x)))
      (cond ((not (eq? z x))
             (tex2page-string z))
            ((do-tex-prim (find-corresp-prim z)) =>
             (lambda (y)
               (if (eqv? y ':encountered-undefined-command)
                   (emit x))))))))
...
(define do-tex-ctl-seq
  (lambda (x)
    ;process ctl-seq x.  Return :encountered-bye,
    ;:encountered-endinput if x = \bye, \endinput
    ;respectively
    (trace-if *tracingcommands?* x)
    (let ((z (resolve-defs x)))
      (cond ((not (eq? z x))
             (trace-if *tracingmacros?*
               "    --> " z)
             (toss-back-char *invisible-space*)
             (toss-back-string z))
            ((and *doing-urlh?* (string=? z "\\\\"))
             (egroup))
...
(define do-if-file-exists
  (lambda ()
    (let* ((file (ungroup (get-group)))
           (thene (ungroup (get-group)))
           (elsee (ungroup (get-group))))
      (toss-back-string
       (if (file-exists? (actual-tex-filename file)) thene elsee)))))

(define check-input-file-timestamp
  (lambda (f)
    ;don't let last-modified-time be affected by
    ;files marked as boilerplate,
    ;and by file-in-cwd specified as ./file-in-cwd
    (unless (or *inputting-boilerplate?*
                (> *html-only* 0)
...
(define do-input-if-file-exists
  (lambda ()
    (let* ((f (ungroup (get-group)))
           (then-txt (ungroup (get-group)))
           (else-txt (ungroup (get-group))))
      (tex2page-string
        (if (do-input-aux (actual-tex-filename f))
            then-txt
            else-txt)))))
  ...
  (lambda ()
    ...
    (let ((f (let ((c (snoop-actual-char)))
      ...
      (let ((boilerplate-index *inputting-boilerplate?*))
        ...
        (if (eqv? *inputting-boilerplate?* 0)
            (set! *inputting-boilerplate?* #f))
        (fluid-let ((*inputting-boilerplate?*
                      (and boilerplate-index (+ boilerplate-index 1))))
          (cond ((and (or (string=? f "texinfo") (string=? f "texinfo.tex"))
                      (not (eqv? *tex-format* 'texinfo)))
                 (set! *tex-format* 'texinfo)
                 (unless (find-def "\\ifinfo")
                   (let ((texinfo-t2p
                           (actual-tex-filename "texinfo.t2p")))
                     (unless (file-exists? texinfo-t2p)
                       (terror 'do-input
                         "Put texinfo.t2p in TIIPINPUTS"))
                     (do-input-aux texinfo-t2p)))
                 (write-log #\space)
                 (write-log "{Rest of ")
                 (write-log *current-tex-file*)
                 (tex2page-file (untexinfo-rest-of-doc)))
                ((or (latex-style-file? f)
                     (ormap (lambda (z) (string=? f z))
                       '("eplain" "eplain.tex" "tex2page" "tex2page.tex")))
                 ;don't process .sty files,
                 ;eplain.tex and tex2page.tex
                 #f)
                (else (do-input-check-bye
                        (actual-tex-filename f)))))))))

(define do-input-check-bye
  (lambda (f)
    (let ((r (do-input-aux f)))
      (when (and (eqv? f *main-tex-file*)
                 (not (eqv? r ':encountered-bye)))
        (trace-if #t "Added missing \\end"))
      r)))
...
(define do-input-aux
  (lambda (f)
    ...
    (write-log #\() 
    (write-log f)
    (let ((r (cond ((file-exists? f)
                    ;don't process non-existent files
                    (let ((t2p-file? (let ((e (file-extension f)))
                                       (and e (string=? e ".t2p")))))
                      ;process .t2p files but don't use their timestamp
                      ;and don't un-texinfo them
                      (unless t2p-file?
                        (check-input-file-timestamp f)
                        (when (eqv? *tex-format* 'texinfo)
                          (set! f (texi-to-tex f))))
                      (tex2page-file f)))
                   (else (write-log #\space)
                     (write-log "not found")
                     #f))))
      (write-log #\))
      r)))
    ...
    (let ((f (ungroup (get-group))))
      (when (or (eq? *includeonly-list* #t)
        ...
        (fluid-let ((*subjobname* (file-stem-name f))
                    (*img-file-count* 0)
                    (*imgdef-file-count* 0))
          (do-input-check-bye (actual-tex-filename f)))))))
...
(define texi-to-tex
  (lambda (f)
    ...
    (call-with-input-file/buffered f
      (lambda ()
        (untexinfo-rest-of-doc)))))

(define untexinfo-rest-of-doc
  (lambda ()
    (write-log #\space)
    (write-log "->")
    (write-log #\space)
...
(define do-evalt
  (lambda (i)
    ...
    (let ((scratch-tex-file
            (string-append *eval-file-stem* ".tex")))
      (fluid-let ((*after-eval* #f))
        (with-output-to-file scratch-tex-file
          (lambda ()
            (do-evalh i)))
        (when *after-eval* (*after-eval*))
        (move-aux-files-to-aux-dir *eval-file-stem*)))))

(define do-eval
  (lambda (fmts)
    (when (> *html-only* 0) (set! fmts 'html))
    (let ((scheme-expr (ungroup (get-group))))
      (unless (inside-false-world?)
        ((case fmts
           ((html) do-evalh)
           ((both) do-evalt))
...
(define call-bibtex-makeindex-if-necessary
  (lambda ()
    (when (and (memv 'bibtex *external-programs*)
               (file-exists? (string-append *aux-dir/* *jobname*
                               *bib-aux-file-suffix* ".aux"))
               (not (file-exists? (string-append *aux-dir/* *jobname*
                                    *bib-aux-file-suffix* ".bbl"))))
      ...
      (write-log #\newline))
    ;
    (when (and (memv 'makeindex *external-programs*)
               (file-exists? (string-append *aux-dir/* *jobname*
                               *index-file-suffix* ".idx"))
               (not (file-exists? (string-append *aux-dir/* *jobname*
                                    *index-file-suffix* ".ind"))))
...
(define make-target-dir
  (lambda ()
    (let ((hdir-file
            (let ((f (string-append *jobname* ".hdir")))
              (if (file-exists? f) f
                  (let ((f ".tex2page.hdir"))
                    (if (file-exists? f) f
  ...
  (lambda (f)
    (when (and *aux-dir*
      ...
      (case *operating-system*
        ...
        ((windows) (system (string-append *shell-cmd-prefix*
                             "copy " f ".* " *aux-dir*))
         (when (or (file-exists? (string-append f ".tex"))
                   (file-exists? (string-append f ".scm")))
           (system (string-append *shell-cmd-prefix*
                     "del " f ".*"))))))))
...
(define !using-external-program
  (lambda (x)
    (set! *external-programs*
      (cons x *external-programs*)))) 
    ...
    (let ((n (string-length f)))
      (if (= n 0) #t
          (let ((c0 (string-ref f 0)))
            (if (char=? c0 #\/) #t
                (if (or (= n 1) (not (eqv? *operating-system* 'windows))) #f
                    (if (and (char-alphabetic? c0)
                             (char=? (string-ref f 1) #\:)) #t
                        #f))))))))

(define ensure-url-reachable
  (lambda (f)
    (if (and (not (fully-qualified-url? f)) *aux-dir*)
        (let ((real-f (string-append *aux-dir*
          ...
          (when (and (file-exists? f)
            ...
            (case *operating-system*
              ...
              ((windows)
               (system (string-append *shell-cmd-prefix* "copy/b " f
                         " " real-f)))))
          real-f)
        f)))
...
(define tex2page-file
  (lambda (f)
    (let ((r (call-with-input-file/buffered f
               (lambda ()
...
(define tex2page-help
  (lambda (not-a-file)
    ...
    (cond 
          ((string=? not-a-file "--version")
           (write-log "Copyright (c) 1997-2002, Dorai Sitaram.
...
(define tex-def-math-prim
  (lambda (cs th)
    (set!texframe.definitions *math-primitive-texframe*
      (cons (cons cs th)
        (texframe.definitions *math-primitive-texframe*)))))
...
'(tex-def-math-prim "\\alpha"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\alpha{$\\alpha$}")
                     (tex2page-string "\\alpha")))
(tex-def-math-prim "\\beta" (lambda () (emit "<i>&szlig;</i>")))
'(tex-def-math-prim "\\gamma"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\gamma{$\\gamma$}")
                     (tex2page-string "\\gamma")))
'(tex-def-math-prim "\\delta"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\delta{$\\delta$}")
                     (tex2page-string "\\delta")))
'(tex-def-math-prim "\\epsilon"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\epsilon{$\\epsilon$}")
                     (tex2page-string "\\epsilon")))
'(tex-def-math-prim "\\zeta"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\zeta{$\\zeta$}")
                     (tex2page-string "\\zeta")))
'(tex-def-math-prim "\\eta"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\zeta{$\\zeta$}")
                     (tex2page-string "\\zeta")))
'(tex-def-math-prim "\\theta"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\theta{$\\theta$}")
                     (tex2page-string "\\theta")))
(tex-def-math-prim "\\iota" (lambda () (emit "<i><small>I</small></i>")))
(tex-def-math-prim "\\kappa" (lambda () (emit "<i><small>K</small></i>")))
'(tex-def-math-prim "\\lambda"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\lambda{$\\lambda$}")
                     (tex2page-string "\\lambda")))
(tex-def-math-prim "\\mu" (lambda () (emit "&micro;")))
'(tex-def-math-prim "\\nu"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\nu{$\\nu$}")
                     (tex2page-string "\\nu")))
'(tex-def-math-prim "\\xi"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\xi{$\\xi$}")
                     (tex2page-string "\\xi")))
'(tex-def-math-prim "\\pi"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\pi{$\\pi$}")
                     (tex2page-string "\\pi")))
'(tex-def-math-prim "\\rho"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\rho{$\\rho$}")
                     (tex2page-string "\\rho")))
'(tex-def-math-prim "\\sigma"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\sigma{$\\sigma$}")
                     (tex2page-string "\\sigma")))
'(tex-def-math-prim "\\tau"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\tau{$\\tau$}")
                     (tex2page-string "\\tau")))
'(tex-def-math-prim "\\phi"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\phi{$\\phi$}")
                     (tex2page-string "\\phi")))
'(tex-def-math-prim "\\varphi"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\varphi{$\\varphi$}")
                     (tex2page-string "\\varphi")))
'(tex-def-math-prim "\\chi"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\chi{$\\chi$}")
                     (tex2page-string "\\chi")))
'(tex-def-math-prim "\\psi"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\psi{$\\psi$}")
                     (tex2page-string "\\psi")))
'(tex-def-math-prim "\\omega"
                   (lambda ()
                     (tex2page-string "\\global\\imgdef\\omega{$\\omega$}")
                     (tex2page-string "\\omega")))

;(tex-def-math-prim "\\alpha" (lambda () (emit "<i>&aelig;</i>")))
;(tex-def-math-prim "\\gamma" (lambda () (emit "<i><small>Y</small></i>")))
;(tex-def-math-prim "\\epsilon" (lambda () (emit "<i><small>E</small></i>")))
;(tex-def-math-prim "\\zeta" (lambda () (emit "<i>&ccedil;</i>")))
;(tex-def-math-prim "\\nu" (lambda () (emit "<i><small>V</small></i>")))
;(tex-def-math-prim "\\pi" (lambda () (emit "<i><b>&ntilde;</b></i>")))
;(tex-def-math-prim "\\rho" (lambda () (emit "<i>&thorn;</i>")))
;(tex-def-math-prim "\\sigma" (lambda () (emit "<i>&oacute;</i>")))
;(tex-def-math-prim "\\tau" (lambda () (emit "<i><small>T</small></i>")))
;(tex-def-math-prim "\\omega" (lambda () (emit "<i><small>W</small></i>")))
...
;(tex-def-math-prim "\\Phi" (lambda () (emit "&Oslash;")))
...
(tex-def-math-prim "\\uparrow"
  (lambda ()
    (emit "<table><tr><td>^</td></tr><tr><td>|</td></tr></table>")))
...
;(tex-def-prim "\\center" (lambda () (emit "<p><div align=center>")))
...
(tex-def-prim "\\description" (lambda () (do-end-para) (emit "<ul>")))
...
(tex-def-prim "\\dontuseimgforhtmlmath"
  (lambda () (set! *use-img-for-math?* #f)))
(tex-def-prim "\\dontuseimgforhtmlmathdisplay"
  (lambda () (set! *use-img-for-display-math?* #f)))
(tex-def-prim "\\dontuseimgforhtmlmathintext"
  (lambda () (set! *use-img-for-in-text-math?* #f)))
...
;(tex-def-prim "\\endcenter" (lambda () (emit "</div><p>")))
(tex-def-prim "\\endcenter" do-end-block)
(tex-def-prim "\\enddescription" 
              (lambda () (do-end-para) (emit "</ul>") (do-para)))
(tex-def-prim "\\endenumerate"
  (lambda () (do-end-para) (emit "</ol>") (do-para)))
(tex-def-prim "\\endfigure" do-end-figure)
;(tex-def-prim "\\endflushleft" (lambda () (emit "</div><p>")))
;(tex-def-prim "\\endflushright" (lambda () (emit "</div><p>")))
...
;(tex-def-prim "\\endtexonly" (lambda () (toss-back-string "\\fi ")))
(tex-def-prim "\\endthebibliography" (lambda () (emit "</table>") (do-para)))
(tex-def-prim "\\enspace" (lambda () (emit-nbsp 2)))
(tex-def-prim "\\enumerate" (lambda () (do-end-para) (emit "<ol>")))
...
(tex-def-prim "\\ifodd" do-unknown-if)
...
(tex-def-prim "\\itemize" (lambda () (do-end-para) (emit "<ul>")))
...
(tex-def-prim "\\noad" 
              (lambda () (set! *self-promote?* #f)))
(tex-def-prim "\\nocite" do-nocite)
(tex-def-prim "\\node" do-node)
(tex-def-prim "\\notimestamp"
  (lambda () (set! *timestamp?* #f)))
(tex-def-prim "\\nr" (lambda () (do-cr "\\nr")))
(tex-def-prim "\\number" do-number)
...
(tex-def-prim "\\picture" (lambda () (do-latex-env-as-image "picture" 'inline)))
...
(tex-def-prim "\\epsfxsize" eat-dimen) ;preamblize
  ...
  (lambda (tex-file)
    (fluid-let ((*after-eval* #f)
               ...
               (when (file-exists? *main-tex-file*)
                 (set! *jobname* (file-stem-name *main-tex-file*))
                 (make-target-dir))
               ;\secnumdepth may be accessed by
               ;load-aux-file
               (tex-def-count-global "\\secnumdepth" -2)
               (load-aux-file)
               ...
               (write-log #\)) 
               (write-log #\newline)
               (cond ((not (file-exists? *main-tex-file*))
                      (tex2page-help tex-file))
                     (else
                      (set! *subjobname* *jobname*)
                       ...
                       (set! *html-page*
                         (let ((ext (file-extension *main-tex-file*)))
                           (string-append
                             *aux-dir/* *jobname*
                             (if (and ext (string-ci=? ext ".html") 
                                      (not *aux-dir*))
                                 "-SAVE.html" ".html"))))
                       (ensure-file-deleted *html-page*)
                       (set! *html* (open-output-file *html-page*))
                       ;(set! *log-file* 
                       ;  (string-append *aux-dir/* *jobname* ".hlog"))
                       ;(ensure-file-deleted *log-file*)
                       ;(set! *log-port* (open-output-file *log-file*))
                       ;(load-aux-file) 
                       (call-bibtex-makeindex-if-necessary)
                       ;(update-last-modification-time *main-tex-file*)
                       (do-start)
                       (let ((t2p-file (actual-tex-filename
                                         (string-append *jobname* ".t2p"))))
                         (when (file-exists? t2p-file)
                           (fluid-let ((*html-only* (+ *html-only* 1)))
                                      (do-input-aux t2p-file))))
                       (do-input-check-bye *main-tex-file*)
                       (do-bye)))

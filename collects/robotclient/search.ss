(module search mzscheme
  ...
  (define (generate-first-moves x y packages)
    (let ((maker (move-maker (weight-from-goal 1) (make-cord x y) 1 #f)))
      (let-values (((p-weight p-bid _ p) (calc-weight 'P x y (current-player) packages))
                   ((d-weight d-bid d __) (calc-weight 'D x y (current-player) null))
                   ((s-weight s-bid) (if (> x 1) (get-move-weight (sub1 x) y) (values void void)))
                   ((n-weight n-bid) (if (< x (board-height)) (get-move-weight (add1 x) y) (values void void)))
                   ((w-weight w-bid) (if (> y 1) (get-move-weight x (sub1 y)) (values void void)))
                   ((e-weight e-bid) (if (< y (board-width)) (get-move-weight x (add1 y)) (values void void))))
        (list (maker p-weight x y p-bid 'P p)
              (maker d-weight x y d-bid 'D d)
              (unless (void? n-weight) (maker n-weight (add1 x) y n-bid 'N null))
              (unless (void? s-weight) (maker s-weight (sub1 x) y s-bid 'S null))
              (unless (void? e-weight) (maker e-weight x (add1 y) e-bid 'E null))
              (unless (void? w-weight) (maker w-weight x (sub1 y) w-bid 'W null))))))          
  ...
  (define (generate-moves x y back back-check g command)
    (let ((maker (move-maker (weight-from-goal g) back g command))
      ...
      (let-values (((d-weight d-bid d _) (calc-weight 'D x y (current-player) null))
                   ((s-weight s-bid) (if (and (> x 1)
                                              (not (and (= old-x (sub1 x))
                                                        (= old-y y))))
                                         (get-move-weight (sub1 x) y) (values void void)))
                   ((n-weight n-bid) (if (and (< x (board-height))
                                              (not (and (= old-x (add1 x))
                                                        (= old-y y))))
                                         (get-move-weight (add1 x) (values void void))))
                   ((w-weight w-bid) (if (and (> y 1)
                                              (not (and (= old-x x)
                                                        (= old-y (sub1 y)))))
                                         (get-move-weight x (sub1 y)) (values void void)))
                   ((e-weight e-bid) (if (and (< y (board-width))
                                              (not (and (= old-x x)
                                                        (= old-y (add1 y)))))
                                         (get-move-weight x (add1 y)) (values void void))))
        (list (maker d-weight x y d-bid)
              (unless (void? n-weight) (maker n-weight (add1 x) y n-bid))
              (unless (void? s-weight) (maker s-weight (sub1 x) y s-bid))
              (unless (void? e-weight) (maker e-weight x (add1 y) e-bid))
              (unless (void? w-weight) (maker w-weight x (sub1 y) w-bid))))))

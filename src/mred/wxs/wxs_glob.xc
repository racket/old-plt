...
@MACRO ubFrameDialog[who] = (((n <= {s}) || XC_SCHEME_NULLP({x})) ? (wxWindow *)NULL : (WITH_VAR_STACK(objscheme_istype_wxFrame({x}, NULL, 1)) ? (wxWindow *)WITH_VAR_STACK(objscheme_unbundle_wxFrame({x}, NULL, 0)) : (WITH_VAR_STACK(objscheme_istype_wxDialogBox({x}, NULL, 1)) ? (wxWindow *)WITH_VAR_STACK(objscheme_unbundle_wxDialogBox({x}, NULL, 0)) : (WITH_VAR_STACK(scheme_wrong_type(<who>, objscheme_modidx, "frame% or dialog%", -1, 0, &{x})), (wxWindow *)NULL))))

...
scheme_expt(int argc, Scheme_Object *argv[])
{
  ...
  if (argv[0] == zeroi) {
    ...
    if (SCHEME_TRUEP(negative_p(1, &e))) {
...
static const char *NICE_SIZE(const char *s)
{
  if (strlen(s) > 255) {
    char *s2;
    s2 = scheme_malloc_atomic(256);
    memcpy(s2, s, 252);
    s2[252] = '.';
    s2[253] = '.';
    s2[254] = '.';
				  ...
				  int test_only)
{
  ...
  while (str[0] == '#') {
    ...
    default:
      if (complain)
	scheme_raise_exn(MZEXN_READ, complain, 
			 "read-number: bad `#' indicator in %s",
			 NICE_SIZE(str));
      return scheme_false;
    }
    str += 2;
  ...
  if ((len > 7) && str[len-1] == 'i') {
	...
	if (complain)
	  scheme_raise_exn(MZEXN_READ, complain, 
			   "read-number: division by zero in %s",
			   NICE_SIZE(str));
	return scheme_false;
      }
...
#define isbaseNdigit(N, ch) (((ch >= 'a') && (ch <= ('a' + N - 11))) \
  ...
  for (i= 0; i < len; i++) {
	...
	if (report)
	  scheme_raise_exn(MZEXN_READ, complain, 
			   "read-number: cannot mix `@' and `i': %s", 
			   NICE_SIZE(str));
	return scheme_false;
      }
      if (i + 1 < len) {
	if (report)
	  scheme_raise_exn(MZEXN_READ, complain, 
			   "read-number: `i' must be at the end: %s", 
			   NICE_SIZE(str));
	return scheme_false;
      }
      has_i = i;
	...
	if (report)
	  scheme_raise_exn(MZEXN_READ, complain, 
			   "read-number: `@' cannot be at start: %s", 
			   NICE_SIZE(str));
	return scheme_false;
      }
      has_at = i;
  ...
  for (i = 0; i < len; i++) {
	...
	if (report)
	  scheme_raise_exn(MZEXN_READ, complain, 
			   "read-number: multiple decimal points: %s", 
			   NICE_SIZE(str));
	return scheme_false;
      }
      if (has_slash) {
	...
	if (!i) {
	  if (report)
	    scheme_raise_exn(MZEXN_READ, complain, 
			     "read-number: cannot begin with `%c' in %s", 
			     ch, NICE_SIZE(str));
	  return scheme_false;
	}
	has_expt = i;
	...
	if (report)
	  scheme_raise_exn(MZEXN_READ, complain, 
			   "read-number: cannot have slash at start: %s", 
			   NICE_SIZE(str));
	return scheme_false;
      }
      if (has_slash) {
	if (report)
	  scheme_raise_exn(MZEXN_READ, complain, 
			   "read-number: multiple slashes: %s", 
			   NICE_SIZE(str));
	return scheme_false;
      }
      if (has_decimal) {
